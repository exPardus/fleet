...
**Process, for the next lane:** the sentinel in the pin is the brief's LAST LINE. "The last line survived" reds for a cap, a slice, a marker misparse, an off-by-one and whatever gets invented next; "the file is big enough" survives all of them. And the census is tied by set-equality to a table of end-to-end DRIVERS, so a new dispatch path cannot be added without owing one — which is what keeps a census from decaying into the allowlist this repo has already paid for once.
