...
> **ROUTING, ADDED 2026-08-09 — READ THIS BEFORE TREATING THIS FILE AS THE LIVE INSTRUMENT.**
> This template was written for the **manager/campaign** shape (one manager, waves of workers, the
> (a)–(h) pipeline). The **supervisor/lane/gate** shape that has run since ~2026-07-26 does not use
> that pipeline, and its doctrine lives elsewhere: **`docs/lanes/BRIEF-TEMPLATE.md`** (the lane brief
> — deliverables, fence, and the `FLEET_HOME`-is-not-a-fence safety stanza), **`docs/lanes/README.md`**
> (report durability), **`skills/fleet/supervisor.md`** + **`skills/fleet/SKILL.md`** (boot ritual,
> bands, succession) and the ratified specs. **If you are writing a lane or gate brief, amend
> `docs/lanes/BRIEF-TEMPLATE.md`, not this file.**
> This note exists because the fold for waves 44–48 measured the cost of its absence: **`lessons.md`
> took 37 commits between 2026-07-25 and 2026-08-09 while this file took zero**, even though §8 below
> makes amending it *required to close* a knowledge wave. The process-change channel had not died —
> it had **moved**, and this file went on declaring itself the living instrument, so every "amend the
> template" instruction pointed into a dead drop. Sections 2–8 remain correct doctrine for campaign-shaped
> work and are cited by live briefs; **§1's "`fleet doctor` must be clean" precondition is the one item
> known to be stale** (doctor has carried deliberate, accounted-for FAILs — a held pin stamp, an
> operator decision slot — for nine consecutive waves; read `docs/OPERATOR-GATES.md` and the doctor
> rows, do not block on a clean board). See `lessons.md#2026-08-09-fold-44-48`.

**Version 1.11 (2026-08-09)** — waves 44–48 amendments, folded late and all from adversarial-gate work. **(1) A GATE BRIEF MAY NAME WHAT TO ATTACK AND MAY NEVER NAME WHAT GRADE THE RESULT DESERVES** (§3 e). Wave 48's brief pre-committed BLOCKING on one attack; the gate graded it CLEARED on the measurement and wrote *"manufacturing a BLOCKING finding to match the a3 precedent would have been this gate's failure mode."* This is v1.x's `a brief that pre-commits a severity is choosing the verdict` recurring **because the campaign now supplies the pressure** — a previous gate's live fail-open became "the standing expectation" in a later brief. **(2) A MUTANT THAT GOES GREEN ON A SUBSET IS NOT A SURVIVOR UNTIL THE FULL SUITE HAS HAD ITS CHANCE** (§5). Escalate every subset-green plant to the full floor, against a clean full floor measured first — wave 48 reported three genuine survivors of a 4217-test suite this way, and **the audited lane's own 6-mutant table was truthful, 6/6 dead**: a self-reported table can be truthful *and* incomplete, with the gap clustering on whatever the author built last. **(3) A PIN THAT SHIPS A CARVE-OUT SHIPS THE CENSUS OF WHAT THE CARVE-OUT EXEMPTS, PER SURFACE, IN ITS OWN DOCSTRING** (§3 e). A dated-line exemption removed *every* candidate line on one of a pin's two parametrised surfaces, so that param was unreachable and its green read as coverage; the mutant survived 713 tests while a control on an undated line reddened at once. **(4) A CONTROL MUST INCLUDE THE SHAPE OF THE DATA — extended to counting controls** (§5). A citation repoint left a backwards range (`7443-7343`); the lane's control was *"the count of `:NNNN` tokens is unchanged"* and `-7343` is not that shape, so a count could not see it. **A count-of-tokens control is blind to any defect living in a token shape it does not count.** **(5) A PLANT DRIVER ASSERTS ITS OWN PATCH APPLIED *AND* CARRIES AN ON-DISK RECOVERY GUARD** (§5). The assert half was already doctrine; wave 48 added the other half when a **10-minute tool timeout killed a planter mid-run and stranded a mutant on disk** — caught by `git status` plus a whole-tree sha256 manifest. The thing that kills a planter may not be its own code. **(6) `FLEET_HOME` IS NOT A FENCE, and `--fleet-home` alone is not either** (§6). Inside a fleet-launched session the sid→home lookup outranks the validated env; and `--fleet-home` **cannot name a home you are about to `init`**, because `fleet init` does not create `state/fleet.json`. The working fence is **unsetting `CLAUDE_CODE_SESSION_ID` in the child environment**, verified by running `fleet home` the identical way. **(7) ASK A CONTAINED LANE *WHY* IT WAS CONTAINED, NEVER MERELY *WHETHER*** (§6). A clean containment audit came back clean because `~/.claude/fleet-homes.list` did not exist, not because the fence held. **(8) A SUCCESSOR QUEUE CITES A STILL-RUNNING GATE BY ARTIFACT PATH AND STATUS, NEVER BY TRANSCRIBED PEEKS** (§8). Two peek-derived findings written into a stand-down queue were contradicted by the committed verdict — *a peek is a gate's working state, and a mid-run alarm is systematically louder than the final grade.* **(9) ORDER A PROOFREADING PASS OVER ANY MEASUREMENT-GRADED DOCUMENT** (§4 doc variant). Measured by withholding a known garbled phrase from a gate: seven findings, not among them. Neither the gate shape nor the suite reads prose for sense — and the class is wider than grammar: *"often works"*, *"for a wave"* and an unpinned link count are **unquantified quantities wearing the clothes of measurements**. See `lessons.md#2026-08-09-fold-44-48`, `#2026-08-09-w48-fences`, `#2026-08-09-w46-fidelity`, `#2026-08-09-w45-ceiling-pins`.
**Version 1.10 (2026-07-24)** — fault-injection restore amendments (§5), both from the same day's live work. (1) **A byte-restore proof may not compare `git show HEAD:<file>` to the worktree file**: this repo is `core.autocrlf=true`, the blob is LF and the file is CRLF, so those two sha256s differ on every line of a **verifiably clean** tree — a clean restore reads as DIRTY and the campaign hunts a phantom. Compare EOL-normalised, or make `git status --porcelain` the authority. (2) **Commit before you inject**: `git checkout -- <file>` reverts the whole uncommitted file, taking the real work out with the injection (paid for once in `fix/handoff-seams`). See `lessons.md#2026-07-24-day3`.
...
- [ ] **2026-07-24 amendment — a byte-restore proof must not compare `git show` output to the worktree file.** This repo is `core.autocrlf=true`, so the blob git hands back is LF and the file on disk is CRLF: on a **verifiably clean** tree (`git status --porcelain` empty) `git show HEAD:bin/fleet.py | sha256sum` and `sha256sum bin/fleet.py` differ on every one of its ~12k lines. A fault-injection round that ends "mutate → red → restore → prove byte-identical" with those two hashes reads a **CLEAN restore as DIRTY** and sends the campaign hunting a phantom. Compare EOL-normalised, or make **`git status --porcelain` the authority** (it is the one check that already knows the filter). Source: tomb-rb micro-confirmation gate, 2026-07-24.
- [ ] **Same day, same worktree, the sharper edge:** `git checkout -- <file>` to undo a fault injection reverts the **whole uncommitted file**, injection and real work together. **Commit before you inject** — then `git checkout --` is exactly the restore you meant.
...
- [ ] **Amend the LIVE instrument with the campaign's friction** — bump the version line, date it, and fold in whatever this campaign proved that the instrument did not yet say. This is the mechanism that keeps it alive.
      - [ ] **2026-08-09 amendment — pick the right instrument first, and say which one you amended.** Campaign-shaped work amends THIS file. **Lane/gate/supervisor-wave work amends `docs/lanes/BRIEF-TEMPLATE.md`** (and, for boot/band/succession doctrine, `skills/fleet/supervisor.md`). Amending neither is the measured default failure: **37 `lessons.md` commits to zero template commits between 2026-07-25 and 2026-08-09.** A lessons entry whose named process change was applied to no instrument at all has **not** discharged this item — record the file and the version you bumped, or record explicitly that the lesson has no mechanical remedy and why.
