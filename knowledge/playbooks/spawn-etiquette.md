...
- **Budget caps.** Put `--max-budget-usd` on any unbounded or open-ended task. Live-tested overshoot: the cap is enforced (the turn is cut short with `error_max_budget_usd`) but is checked between generation chunks/turns, not throttled mid-stream — observed spend ~3x the nominal cap on a tiny ($0.01) budget. Treat it as a circuit breaker that bounds a runaway worker to "a few times the cap," not a precise ceiling.
