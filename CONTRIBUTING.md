...
- Python via `py -3.13` (bare `python` on this project's dev machine resolves to 3.10 — always use `py -3.13`, or your own Python 3.13 on `PATH` elsewhere).
- `bin/fleet.py` is stdlib-only, single file. No pip dependencies get added to it or to the hook scripts under `bin/hooks/`.
- Claude Code CLI ≥ 2.1.202 if you're running live-integration tests.
