...
- **Three claim shapes in the entry docs are pinned to code rather than to prose.** `tests/test_doc_claims.py` re-derives the doctor check count, the Python floor and the set of shipped verbs from `bin/fleet.py`, and checks them against `README.md`, `docs/getting-started.md`, `docs/concepts.md`, `docs/README.md`, `docs/launch-readiness.md` and this file. The check count is why: it drifted twice — `21`→`23` fixed by hand in 2026-07-23's doc pass, rotted to `25`/`22`/`23` against an actual `28` two weeks later. **The pin is deliberately narrow, and its module docstring lists what it does *not* hold.** The three limits to keep in your head: it catches a verb the docs name that does not exist, but never a shipped verb the docs forgot; it checks no flag against any verb; and each shape is held in its canonical phrasing only, so dropping the backticks takes a claim out of its view. So it will catch you renaming a verb or moving the check count, and it will *not* catch you adding a verb nobody documented. Every other number in these docs — test counts, file counts, subcommand counts — is still hand-pasted and unheld.
