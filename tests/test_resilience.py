...
class TestParseLimitSignalLocalFormat:
    ...
    @pytest.mark.parametrize("text", [
        "resets 4am (soon)",       # prose aside must not match
        "resets 4am (approx)",
    ])
    def test_single_segment_arm_rejects_arbitrary_words(self, text):
        assert fleet._LIMIT_RESET_LOCAL_RE.search(text) is None

    def test_single_segment_allowlist_is_case_insensitive(self):
        """Reconcile merge: this case USED to be `"resets 4am (utc)"` in the
        parametrize above, asserting the D4 arm's case-SENSITIVITY (its
        `(?-i:[A-Z][A-Za-z]+)` group). ND-5 superseded that arm with a CLOSED
        fixed-UTC allowlist under the pattern's own IGNORECASE -- see
        TestParseLimitSignalSingleSegmentZone. Case no longer carries any
        safety load: it was a proxy for "looks like a tzdb key", and the
        allowlist decides membership directly. `(utc)` means UTC in every
        season exactly as `(UTC)` does, so admitting it is the intended
        behaviour, not a regression -- pinned here rather than dropped."""
        assert fleet._LIMIT_RESET_LOCAL_RE.search("resets 4am (utc)") is not None
