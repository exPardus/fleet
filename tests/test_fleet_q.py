...
def _shard_path_if_it_were_spellable(root, rel):
    """Where `shard_path_for_source`'s FORMULA lands for a rel the index now
    refuses to spell at all.

    MERGE NOTE (idx/q x idx/core). The containment tests below need to name the
    file a regression WOULD write or unlink outside the root, and they used to
    get it by asking `shard_path_for_source` -- honest at this branch's base,
    where `_index_posix_rel` returned any rel unchanged. `idx/core` (`3e47642`)
    made that primitive RAISE `IndexPathError` on a `..` segment, so it can no
    longer serve as the oracle for its own attack: the question "where would
    this land" now gets an exception instead of a path.

    So the formula is mirrored here with the two refusals omitted -- and only
    them: the empty/`.` segment filter is kept, so this stays
    `_index_posix_rel` minus its guards rather than a second, drifting spelling
    of a rel. Built from `index_symbols_dir` and `INDEX_SHARD_SUFFIX` rather
    than hardcoded, so it still follows the shard tree if that tree moves --
    which is the property the old `shard_path_for_source` call was chosen for.

    Using this in a test is not a hole in the guard. Nothing in `bin/fleet.py`
    reaches the shard formula without passing `_index_posix_rel` first; this
    helper is a test-side ruler for a location, and every assertion it feeds is
    that the location stays UNTOUCHED."""
    parts = [p for p in str(rel).replace("\\", "/").split("/")
             if p not in ("", ".")]
    return fleet.index_symbols_dir(root) / ("/".join(parts)
                                            + fleet.INDEX_SHARD_SUFFIX)
...
class TestOutlineShardPathContainment:
    ...
    def _escaped(root, name):
        ...
        moves. See `_shard_path_if_it_were_spellable` for why the computation no
        longer goes through `shard_path_for_source` itself."""
        shard = _shard_path_if_it_were_spellable(
            root, f"../../../gp/par/proj/{name}")
    ...
    def test_the_guard_also_refuses_a_rel_that_only_leaves_the_shard_tree(
        ...
        stray = Path(os.path.normpath(
            str(_shard_path_if_it_were_spellable(jail, rel))))
...
class TestRowOrderAcrossARefresh:
    """`verified_shard_rows` returns rows in SHARD ORDER (`render_shard` sorts
    by `(line, end, name)`) whether or not it just refreshed. It once answered
    from parse order on a refresh, which diverged on any `(line, end)` tie --
    and `ZED = ALPHA = 1` is one.

    That was an `idx/core` defect, not a `q` defect, and `idx/core` FIXED it
    (merged to `main` as `3e47642`). The characterisation test that pinned the
    instability is retired; the tests below pin the fixed property forward, on
    the same tie fixture that used to expose it."""
    ...
    def test_outline_order_is_stable_across_a_refresh(self, tie, capsys):
        """The forward pin that replaced this file's characterisation test.

        `--outline` renders `render_digest` over the primitive's rows
        directly, because §7 defines the digest as a rendering of the SHARD
        and the shard is sorted by line. `q` deliberately does NOT re-sort in
        this output layer -- so this golden reads the primitive's order
        straight through, and a regression in `verified_shard_rows` lands here
        rather than being hidden by a second sort.

        A `(line, end)` tie (`ZED = ALPHA = 1`) is the case that used to
        diverge: a refreshing read answered from parse order. `idx/core`
        (`3e47642`) made shard order unconditional, so all three reads --
        cached, refreshing, cached again -- now agree."""
        ...
        # ...and the read AFTER the refresh landed is cached again, so it must
        # agree too: the old defect flipped between these two.
