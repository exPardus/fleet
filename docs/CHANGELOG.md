...
w98 (671878f): native Codex lanes through mcx (item 29), landed a day before the 2026-09-20T18:16Z reset. `fleet spawn --model codex:<model>` dispatches a Codex worker through mcx and records `substrate: "codex"`, `dispatch_kind: "mcx"` and the mcx_id; status probes `mcx result`, peek tails the job log, send steers over stdin (refuses a live turn — steering restarts the run), interrupt/kill are `mcx stop`, respawn stops the old job and stamps a new mcx_id, and wave accounting reads token cost from the registry row's job directory. A Codex worker cannot commit under mcx's default `workspace-write` sandbox (the worktree gitdir lives outside the cwd), so `fleet land` commits on the worker's behalf — documented in SKILL.md. 8 files, +1251/-46, 28 stub-mcx tests, no Codex API call; floors 5445 passed / 6 documented host-venv failures on 3.10 and 3.12. Model: openrouter:deepseek/deepseek-v4.1-flash; reviewer findings: 0. Supervisor notes: third body on the lane (w98, w98c archived by reap, w98d); w99 rebase conflict resolved by supervisor (1 semantic union in the spawn record — codex dispatch_kind + w99's branch join key — plus repoint, suites re-green).
w100 (b0923d8): `fleet respawn` accepts a dead-suspected row whose stream died (item 31), no `--force` needed — the common OpenRouter death that made the safe verb require the unsafe flag twice in one day. Acceptance requires BOTH verdicts: probe says dead-suspected AND the transcript tail's newest qualifying record is a stream death (`stream closed before completion`, `Connection lost mid-response`, `Server error mid-response`, `The response stopped arriving`); 429/402/model_not_found excluded, a genuinely running turn still requires `--force`. 12 tests red-first, targeted 525 passed both pythons, floors green with the same six host-venv artifacts. Model: openrouter:deepseek/deepseek-v4.1-flash; reviewer findings: 0. One steer: lane report trimmed 77 to 38 lines (REPORT_LIMIT).
Accounting: OpenRouter key spend USD 42.74 lifetime, USD 41.95 today (auth/key endpoint, 2026-09-19) — +9.78 since wave 88 on two deepseek lanes plus supervision; roughly USD 40 of the resumed ~50 remains. Queue item 35 (hotfix merges trip UNJOINED) filed at wave 88 remains open.
...
w99 (4c14d34): the merge subject now joins to its lane and the join key survives the tidy-up (items 26 and 16). `fleet land` prints the branch form `merge(w99/lane-join): ...`; the registry record keeps the dispatch branch (read from the worktree's own HEAD, no subprocess) and `_wave_lane_join` joins by branch first, worktree second; a parsed-but-unjoined lane refuses the close as `UNJOINED: N of M`; an unreadable substrate makes tokens and tokens_per_bin_line UNMEASURED instead of wave 86's confident `tokens: 0` on a 701-line wave. 9 files, +586/-100, 9 new/changed tests all red-first; targeted suites (test_wave_close, test_core, test_fleet_land) 165 passed / 1 skipped on 3.10 and 3.12. Model: openrouter:deepseek/deepseek-v4.1-flash; reviewer findings: 0. Supervisor notes: joined-but-no-token-source verified covered — `_wave_outcomes_claude_tokens` returns UNMEASURED for missing session, missing ledger, missing usage and partial session match; only a full match sums.
Accounting: OpenRouter key spend USD 32.96 lifetime, USD 32.17 today (auth/key endpoint, 2026-09-19); account credits ~USD 50 of 100 remain per the interface. Also in this wave: item 27a wake-after-seize fix (ed7de53, ee7e8f0, ec98a7d) pushed.
...
w97 (69220b3): `fleet sup-spawn` / `sup-handoff-begin --model openrouter:<slug>` now dispatch a supervisor body on OpenRouter (item 28). The dispatch half pre-existed through `dispatch_bg`; this lane pins it by test and builds the recording half: gen-0 and handoff registry rows stamp `substrate=openrouter/<slug>`, `sup-status` reports the body's model/substrate through the read-only registry path, and supervisor journal headers carry an optional `substrate=` token (legacy headers still parse). Missing/unreadable key refuses with the worker path's exact message; claim/nonce/handoff protocols untouched. 13 new tests, 10 red-first; full floor 5423 passed / 6 pre-existing host-venv failures on 3.10 and 3.12. Model: openrouter:moonshotai/kimi-k3; reviewer findings: 0. Supervisor notes: the lane's own registry row records tokens in=None out=None (openrouter body), so any printed token total for this wave is UNMEASURED, not 0.
...
w92 (edd30f6): `fleet wave-close` now stops the native session of every lane it marks `lane_state=landed`, through the sanctioned `_stop_native_session_status` path and never under `fleet_lock()`, then reaps a second time -- so the reap predicate's lane arm fires inside the same wave-close run and a landed lane frees its host slot immediately instead of holding it until the daemon idles the session out about eight hours later. The live-PID protection in `_reap_protection` is unchanged, and a source-pin test keeps it that way: a real `claude --bg` session stopped with `claude stop` has its roster entry disappear entirely, so there is no live PID left to veto. This closes the long-standing defect that the lane arm had never once fired on a real lane.

w93 (b64bc53): `fleet spawn --model openrouter:<slug>` dispatches a native worker against OpenRouter. Env set on the dispatching process does not reach a `--bg` session (the daemon hands it to a pre-warmed spare), and a `--settings` `env` block never expands `$VAR`; the working path is `apiKeyHelper`, a command the session runs itself at auth time, pointed at the key file and never at its value. The merged settings are passed inline on argv, so no file fleet writes ever contains the credential. A dispatch is refused with the path named when the key file is missing or unreadable, worker records and `docs/lanes/<lane>.json` carry `substrate: openrouter/<slug>`, and wave-close accounts such a lane as a real measured `0` rather than `UNMEASURED`.
