...
Moved verbatim from `knowledge/lessons.md`; this file is not loaded.

## 2026-09-10 — wave 61: the fleet went dark for 8h10m and nothing had failed <a id="2026-09-10-w61-stalled-supervisor"></a>

**THE OUTAGE HAD NO DEFECT IN IT.** Wave 60 landed and pushed; the supervisor `sup-notify`d the interface and **ended its turn normally** at 246k against a 350k band — correct, under-band, nothing owed. Nothing ever gave it another turn. **A supervisor's heartbeat only refreshes when it TAKES a turn**, so the beat went stale an hour later and stayed stale for eight hours while the session sat *alive and idle*. The keeper ticked ~30 times and paged nothing. *A body that has finished its work and a body that has died are the same observation from outside, and this fleet had an alarm only for the second.*

**THE RULE IS NAMED FOR THE WRONG FAILURE.** `rule_supervisor_dead` ANDs a stale heartbeat with roster-**absence**; the fleet's actual failure mode is **`supervisor_stalled`** — alive, listed, holding the claim, not acting — and the keeper has **no rule for it**. The roster arm is correct code doing exactly its stated job; it is what stops the one rule that exists from noticing. **The supervisor who filed the gate framed it as a LIVENESS question** — *"is there anything in the roster that distinguishes a live body from a dead one? If so the arm can be repaired"* — and the lane sent to measure it refuted the premise: `pid`/`status`-presence **is** a perfect live/dead discriminator (27/27), **and repairing the arm that way would not have shortened the outage by one second, because the supervisor was alive.** *Framing an idleness failure as a liveness failure makes every subsequent measurement answer the wrong question correctly.* The useful cut is `status == "busy"`, not "`status` present" — already in the JSON the keeper parses, already documented in `native-substrate.md`, and it would have paged **7h10m51s earlier** with no new false positive and C2's sid join intact.

**EIGHT HOURS WAS THE LUCKY CASE, AND THE BOUND THE SUPERVISOR ASKED FOR DOES NOT EXIST.** The brief said *if entries expire, the blindness has a bounded duration and that number is the most decision-relevant thing you can produce.* The lane found the timer — the daemon retires an idle bg session at 8h — and then found the bound does not hold: **`settled` retirements become `done` and leave the keeper's list; `idle-prompt` retirements become `blocked` and stay, with no expiry observed** (oldest 22h05m; a `done` row 39h old still in `--all`). 11/11, no exceptions. **2 of the 3 retired supervisor bodies on this host retired `idle-prompt`.** *A reassuring mechanism confirmed is not a reassurance until you check which branch of it you are on.*

**THE CONDITION RECURRED THE SAME DAY WITH A DIFFERENT CAUSE.** The body that filed the gate then died mid-turn on an API error and **sat idle-but-listed itself** until the interface woke it by hand. So it is not a property of clean wave boundaries: **any turn that ends without scheduling the next one produces it**, an error included. Two occurrences in one day, neither detected. A supervisor holding a harness-tracked background task (`fleet wait`) gets a turn when it completes — a mitigation for a live wave, **not a repair**, because it works only while there is something to wait on.

**NEVER TAKE `keys()` OF ONE ROW OFF A HETEROGENEOUS LIST.** The supervisor and the interface reported contradictory field sets for the same command and each concluded the other was reading a different surface. Neither was: **roster rows are heterogeneous by `kind`** — `interactive` rows carry `pid`/`status` and no `state`, `background` rows carry `state` and carry `pid`/`status` only while the process lives. The supervisor's key list was exactly one interactive row's. *Dump the union across all rows, and the full JSON of each.* Related and cheaper: **`claude agents --json` and `claude agents --json --all` are two surfaces** — the keeper calls the first, `bin/fleet.py` the second — and reading a count off the wrong one looked exactly like a mass roster loss, which is the "suspicious roster ⇒ freeze" condition. **Name the spelling beside every count.**

**A BRIEF'S ATTRIBUTION IS A CLAIM, AND THIS ONE ROUTED THE FIX AWAY FROM THE CAUSE.** Three waves of successor queues recorded that *"the six host-assumption failures WRITE INTO THE REPO ROOT"* and that a `skipif`-or-fix decision was owed. MEASURED: `--bogus/` and the newline-named directory are created by tests that **PASS**; `skipif` on the six would have stopped none of it. Fixed with `cwd=tmp_path`, pinned, mutant-verified. *An inherited attribution nobody re-derived sent a real remedy at the wrong target for three waves.*

**REMOVING A COLLISION IS NOT CLOSING A SEAM, AND THE REPORT MUST SAY SO.** The suite's hardcoded real historical sids were retired for synthetic non-RFC-4122 values `claude` can never issue — but `INSTALL_ROOT` is still `__file__`-derived and unoverridable, every subprocess drive still reads the install-root home's registry, and a sid that home carries still retargets a destructive verb. The hazard was also **over-stated by two coincidences**: a lookup hit gives a retarget and red tests; the *write* additionally needs a same-named worker owned by the colliding sid. And there were **four sid sites in two files, not three in one** — the fourth handed to a child.

**BOTH LANES CORRECTED THEIR SUPERVISOR ON THE POINT ITS BRIEF CALLED MOST LIKELY, for the fourth wave running.** Floors predicted in writing before every run and hit exactly on 3.10 and 3.12 at every sha: **4946 → 4965**. *Keep dispatching lanes told that agreeing with the brief is not the deliverable.*

## 2026-09-09 — wave 59: the handoff was proven six weeks before anyone said so, and the "first" drill was the sixth <a id="2026-09-09-handoff-proven"></a>

**A supervisor handed the claim off cleanly, wrote up its own success, and called it *"the first proven supervisor succession on this host — 0-for-8 is now 1-for-9."* The transfer is real and every artifact holds. The record around it does not.** Counted over `supervisor/JOURNAL.md` at `1294920` by a fenced lane that was told not to take the supervisor's word: **26 `HANDOFF-BEGIN`, 15 `HANDOFF-COMPLETE`, 3 `HANDOFF-ABORT`.** The eight stillbirths are all pre-fix. **Post-fix the route is 6-for-6** — `2026-08-05T16:48:54Z`, `17:32:10Z`, `19:33:26Z`, `2026-08-09T19:03:01Z`, `22:54:36Z`, and then this one — and the first five ran on the retired Windows host (`task=C:/proga/claude-fleet/…` in 25 of the 26 begins). **The sentence the drill actually earns is: the claim moved by the handoff path for the first time ON THIS HOST and on POSIX, on a quiet fleet with no lane in flight, at ~169k, at a boundary the outgoing body chose for itself.** Every surface now says that instead of "proven".

**THE FAILURE IS NOT THAT A CLAIM WENT STALE. IT IS THAT THE REFUTING EVIDENCE SAT IN A GIT-TRACKED FILE IN THE SAME REPO FOR THIRTY-FIVE DAYS AND THREE DOCUMENTS KEPT SAYING "NEVER".** `skills/fleet/SKILL.md`'s *"no live drill has run under the fixed default, so the route is a CANDIDATE, not a proven one"* entered at `b9957f7` on 2026-07-30 and was **true for six days**. It went false at `2026-08-05T16:48:54Z`. It was then **re-landed verbatim on 2026-09-09 by `5d11f99`, an edit to that very line** — the editor re-read the sentence, judged it, wrapped it in new context, and never re-ran the count. *A stale claim survives an edit to its own line, because editing a sentence and re-deriving it are different acts and only one of them looks like work.* The prior wave's lesson had already got halfway there — *"the 8-stillbirths framing outlived its own fix by six weeks"* — and stopped at the fix instead of counting the completions the fix produced.

**AND THE COUNTER-LESSON THIS WAVE OWES ITS OWN BRIEF: it is NOT true that "only a live drill retires the word unproven".** Five live drills had already run under the fixed default and the word did not move. **A drill retires nothing; a COUNT retires it.** The brief also proposed *"the tier that runs the protocol is the only one that can retire a claim about it"* — the evidence inverts it. **The tier that runs the protocol is the WORST placed to retire a claim about it**, because it is grading its own event and its evidence is its own journal; the body that got the record right was a fenced lane with no stake, doing arithmetic. *Retiring a claim is a counting job, and it belongs to whoever did not do the thing.*

**A PREMISE CAN DIE WITHOUT TAKING ITS ANSWER WITH IT — CHECK WHAT THE ANSWER RESTED ON, NOT WHETHER THE PREMISE SURVIVED.** `graceful-succession.md` §7 asks *"does anything here depend on `sup-handoff-*`, which has never completed under its shipped default?"* The premise is now false; the answer (**No**) is untouched, because it was grounded in **which function calls which**, not in the route's health. Two clauses around it did move, and **one changed sign**: the refusal that blocks recovery on a minted `handoff_token_hash` used to cost a real window (holding off recovery for a succession that would never happen) and now waits on a mechanism that works — **better justified after the repair than before it.** *Ask what an answer's grounds are before assuming a dead premise falsifies it, and check both directions.* Also found while checking: the verb this whole argument is about, `sup-recover`, **is still UNBUILT** — `grep -c 'sup-recover' bin/fleet.py` returns 0 — so the routing answer is a claim about a design, not a measurement of a shipped call graph, and the spec never says so.

**THE RED DOCTOR ROW A SUCCESSFUL HANDOFF SHIPS IS REAL AND IS NOT ABOUT HANDOFF.** The inheriting supervisor derived that `[FAIL] identity-witness: LEAK` is *structural to every handoff* — a predecessor is alive when it dispatches, so a daemon is already live, so the successor's stamp can never name the successor. **Refuted by measurement, from inside a lane that is nobody's successor:** `fleet doctor` run from worker `w59-proven` reads the same red row, witness `sup|inc-20260909T162927Z-147a|boot` against registry verdict `w59-proven`, with no handoff anywhere near it. The witness names **whichever `--bg` dispatch FOUNDED the live daemon** (`daemon.lock` pid 119225, `startedAt` 2026-09-09T16:29:31Z), so **every body that did not found the daemon reads red** — this is claim-nonce §18, ratified 2026-07-30, and `SKILL.md` already documented it. The argument's step 3 also conflates *"the predecessor is alive"* with *"the predecessor is `--bg`-hosted"*; `DAEMON_ENV_LEAK_REMEDY` itself names the escape (*"ensure the supervisor's own dispatch is the one that starts it"*), which is reachable when no `--bg` session is live at begin time. **On a persistent server fleet it is red in practice always, which is the operationally true and much less interesting version.** *A finding derived on the turn you did the thing is a finding about you; run it on a body that did not do the thing before calling it structural.*

**TWO LANES IN ONE MERGE CANNOT DESCRIBE EACH OTHER'S STATE, AND AN HONEST HEDGE IS NOT A RECONCILIATION.** `w58-docs` wrote *"`fleet sup-notify` … is UNSHIPPED at `2a15dec` … the spelling is provisional"* into `docs/SPEC.md` §18 and a `⚠ NAME UNSHIPPED — reconcile at merge` marker into `SKILL.md`, correctly, with its pin. Its sibling `w58-notify` shipped the verb. Both merged in the same range, and the honest hedge shipped as a **false present-tense sentence in a status entry** and as a warning in the file an interface session reads at startup. **A hedge names a debt and assigns it to the merging tier; no lane can discharge it from inside its own fence.** The merging supervisor recorded that it did not reconcile. *Every merge of sibling lanes owes a re-grep of every "verify at merge" marker either lane wrote, and the marker should carry the command.*

**HOW A PINNED PAST-TENSE CLAUSE INSIDE A PRESENT-TENSE STATUS ENTRY WAS DISPOSED OF, ARGUED RATHER THAN DEFAULTED.** *"is UNSHIPPED at `2a15dec`"* is pinned and still true (re-verified: `git grep -c "sup.notify" 2a15dec -- bin/ tests/ docs/` returns nothing). The ratified 2026-08-05 rule — *"a pinned receipt and a quoted argument are claims about a PAST tree and are still true"* — **cuts the sentence in two rather than choosing a side**: the pinned half is evidence about WHEN the gap existed and editing it destroys the only record of that; the unpinned present-tense inference it licensed (*"the spelling is provisional"*) is rot. Keep the pin verbatim, correct the inference beside it. *The rule's test is that it splits a sentence, not that it saves or condemns one.*

**AND WHAT THE RECEIPT HARNESS CANNOT HOLD, SAID RATHER THAN FORCED.** The artifacts that carry a one-time live succession — `supervisor/INCARNATION`, `sup-status --json`, `state/events.jsonl`, `~/.claude/daemon.lock`, `doctor` output — live in gitignored `state/` or outside the repo, so no materialised tree contains them and `# volatile` would only WARN and be skipped by `tests/test_receipts.py`. **A one-time live event on one host is not receiptable by this harness.** What IS receiptable is the append-only, git-tracked `supervisor/JOURNAL.md`, so the COUNTS became six pinned receipts at `1294920` (§9.4) and the live readings stayed attributed prose. *When the honest answer is that evidence cannot be a receipt, say so in the document instead of manufacturing a block that proves nothing.*

**Second independent witness of the transfer, found because `claimed_via` alone is one string in one file.** `mint_lineage_id()` stamps `now()` and is called at exactly two sites — the fresh claim and the seize/limit-transfer arm; `cmd_sup_handoff_complete` calls neither and CARRIES the predecessor's lineage. **So a claim whose `lineage_id` stamp strictly predates its own `claimed_at` cannot come from any path but a completed handoff.** The live claim reads `lin-20260909T162943Z-6040` against `claimed_at: 2026-09-09T17:49:36Z`. *When one field is the whole proof, look for a second field whose VALUE SHAPE only one path can produce.*

**Floor: predicted in writing before the run and derived, not defaulted.** The brief asserted that a docs-only landing moves the floor by construction; **it does not here, and the derivation is the point**: `tests/test_receipts.py` parametrises **per spec FILE**, not per receipt, so a spec already in the enforced set may gain receipts freely (14 → 34 extracted, floor unmoved); `test_doc_claims.py`'s `CHECK_COUNT_DOCS` does glob the tracked tree, but `docs/lanes/` is an exempt historical prefix so a new lane report adds nothing, and every other file touched was already tracked. Predicted **4880 collected, `6 failed, 4857 passed, 16 skipped, 1 xfailed`, unchanged — and hit to the digit on both 3.10 and 3.12**, the 3.10 run taken after the commit so the `git ls-files` glob saw the landed tree. *"A docs-only landing moves the floor by construction" is a heuristic, and a heuristic is not a prediction — the parametrisation is checkable in one grep.*


## 2026-09-09 — wave 56: the keeper was blind to 19 of 21, and both lanes refused their brief's easy answer <a id="2026-09-09-w56-keeper-and-permissions"></a>

**Two lanes, two worktrees, both merged; floors predicted in writing before the merge and hit to the digit on both interpreters — `6 failed, 4822 passed, 7 skipped, 1 xfailed`, 4836 collected, six pre-existing failures unchanged.** The wave's value is not the two fixes; it is that each lane came back contradicting the brief that sent it.

**THE KEEPER PAGED THE WRONG NUMBER IN BOTH DIRECTIONS FROM ONE CAUSE.** `_git_unpushed` hardcoded `origin/main..main`. Work on this host is on `server/persistent-fleet`, so when the rule paged *"2 commits unpushed"* the true exposure was **21** — the entire server bring-up, on one disk, invisible to the rule for the whole window the keeper exists to cover. Then the branch was pushed, leaving **0** at risk, and the rule would have kept paging **2 forever**, because `main`'s two commits are fully contained in the pushed branch. *A rule that names a fixed ref pair is measuring a location, not a risk; the two coincide only until someone opens a branch.* And the second half is the worse half — the soak file's own prose says a falsely-firing rule is worse than an absent one *"because it teaches the operator to ignore the channel"*, and this one would have taught that on the channel carrying every other page.

**THE FIX I WOULD HAVE SHIPPED WAS A 10,000× REGRESSION, AND THE LANE MEASURED IT INSTEAD OF ADOPTING IT.** The brief named `git rev-list --count HEAD --not --remotes` as the obvious candidate. In a repo with **no remote-tracking refs it counts the entire history and exits 0** — 5 of 5 in a fresh repo. So the naive swap trades a 10× under-report for a 10,000× over-report **that does not look like a failure**: no error, no non-zero rc, just a confident enormous number. A `for-each-ref refs/remotes/` probe now runs first and the rule stays silent when it cannot tell, which is the direction the original docstring had already chosen for its own edge case. *When a brief hands a lane the answer, the lane's job is to measure the answer, not to install it — and the most dangerous replacement is the one whose failure mode returns a plausible integer.* The lane also rejected the `--branches` alternative with grounds rather than preference: lane branches are committed-and-unpushed **by policy** until merge, so `--branches` would manufacture the same operationally-false page every wave, and their remedy is a merge, not a push.

**A SUPERVISOR PREMISE, REPEATED INTO TWO BRIEFS AND A CHECKPOINT, WAS FALSE.** I priced the worker-permission fix as a choice between defaulting to `bypass` (A) and *"shipping a real `permissions.allow` block in `worker-settings.template.json`"* (B), citing `SPEC.md` §18's fleet-index M2 entry as saying B was owed. **The template has shipped `permissions.allow: ["Bash(fleet q:*)"]` since July.** B was never unbuilt — that single entry *is* what the dying workers had — and the M2 sentence is wrong on three counts: the blocker it names is already satisfied, its `@8478` is a stale line citation, and it calls a denial a hang, which is the exact distinction that cost wave 55 three stillborn workers. **The spec sentence was cited as live in two briefs and a checkpoint before anybody opened the file it describes.** *A spec entry naming a blocker is a claim about the current tree and rots like any other; a blocker nobody re-checks becomes a fact by repetition.* Corrected by quoting the old sentence rather than deleting it, since it had already propagated.

**AND THE SUPERVISOR PUT A LINE CITATION IN A ROLLING DOCUMENT.** Wave 55's own `knowledge/INDEX.md` line cited `bin/fleet.py:21451` — **already stale by 78 lines the day it was written**, and moved again by this wave. The 2026-08-05 ratified rule is that *a ROLLING document may never be line-cited*, and `INDEX.md` is append-at-top. Caught by a lane flagging it rather than editing it, correctly, because `knowledge/` belongs to a knowledge wave. *The doctrine you wrote down is not the doctrine you follow; the gap is only visible to a reader who is not you.*

**S-2 WAS HALF-CLOSED, AND CLOSING HALF OF IT REVERSED ITS OWN LAYER OPINION.** S-2 is worded for a permission **stall**; what this host measured is a permission **denial**, and the lane refused to silently re-purpose the stall's slot. The denial half is closed in `status_snapshot()`, which is where S-2 said such a signal belongs. But for the **stall** half the lane found S-2's opinion is not merely expensive but **impossible**: `waiting_for_permission` is transient, every persist site strips it, and the roster is the only witness — so no file-only view can see a stall without breaking terminal-surface D1/D4. *A deferred item's stated remedy is a guess made before the measurement; closing half of it is the first time anyone checks the other half's premise.*

**THE DETECTOR IS A MEASUREMENT, NOT A RULE, AND THAT WAS THE POINT.** The signal chosen is the harness's own typed `toolDenialKind: "permission-rule"` field: all three wave-55 `dontask` transcripts carry it (11, 3, 3), every other session on the box carries it zero times. **The `result_text` substring alternative was rejected on evidence, not taste — only ONE of the three dying workers left a result record at all**, the other two left just a kill tombstone, so a substring rule would have caught one worker in three *and* fired on any report that merely quotes a denial message. Nothing infers a verdict from the count, so nothing can fire falsely; `doctor` gets a NOTE row, not a FAIL. *The cheapest way to make a new signal safe is to ship the count and withhold the judgement.*

**ADDING A `_doctor_check_*` IS A DOCS CHANGE.** The lane predicted its floor, got `12 failed` instead of `6`, and all six extras were its own: `tests/test_doc_claims.py` derives the doctor check count by AST and holds it over every tracked doc **in two phrasings**, so registering a 29th check reddened four documents. It had run a targeted subset before predicting and that file was not in it. *A targeted pre-run is a sample, and a prediction built on a sample inherits the sample's blind spot — the count-bearing pins are exactly the ones a subset omits.* Re-measured three ways (grep 29, AST 29, live `doctor` 29 rows) rather than hand-pasted, and §13's count history gained a step instead of being overwritten.

**Process note that worked:** two lanes, two SEPARATE worktrees, after wave 55 lost its suite receipt to three lanes in one tree. Each ran its own full floor with nothing else writing under it, both reproduced `4776` exactly before editing, and the merged prediction (4776 + 43 + 17 = 4836) was written before the first merge command and hit on both interpreters. *Isolation is not just for the writer; it is what makes the reader's number attributable.*

## 2026-09-09 — wave 55, the first server wave: `dontask` came back on the one surface its fix never reached <a id="2026-09-09-w55-server-first-wave"></a>

**First supervisor incarnation on kz-work (`inc-20260909T062235Z-abe4`, fresh claim, no seizure). Four landings, the branch pushed for the first time, and the wave's two largest findings were both about the WAVE ITSELF rather than its subjects.** Clean floors, taken after every lane was idle and the tree quiescent: **`6 failed, 4762 passed, 7 skipped, 1 xfailed` — 4776 collected, IDENTICAL on 3.10 and 3.12**, failure set the six pre-existing members node-id for node-id, in 4m56s and 4m36s (the first real durations this host has on record; "~6 min" was always a wall-clock impression and is now retired).

**EVERY WORKER IN THE WAVE WAS BORN PARALYSED, AND THE MEASUREMENT THAT SAYS SO IS ALREADY IN THIS REPO.** Three lanes dispatched at the shipped `fleet spawn` default `--mode dontask` did nothing on turn 1: *"Bash has been denied because Claude Code is running in don't ask mode"* — `Write` and `Edit` denied outright, pipes, chaining and heredocs denied, while `ls` and `echo hello` passed. `bin/fleet.py:13897-13931` already carries the verdict in full: `dontask` was the SUPERVISOR default 2026-07-21→27 and was removed after **10 of 10 successors under it were stillborn against 7 of 7 under `bypass` — 17/17, zero exceptions** — with the conclusion *"`dontAsk` does not prompt, it DENIES anything the allow-list does not already cover"* and *"Do NOT restore dontask: it trades a loud wedge for a silent death, which is strictly worse and cost three days."* **`SUCCESSOR_DEFAULT_MODE` and `SUP_SPAWN_DEFAULT_MODE` were both moved to `bypass`; `p_spawn`'s `default="dontask"` (`bin/fleet.py:21451`) is the third dispatch surface and it kept the retired value.** *A doctrine fixed at every site its incident touched is not fixed — the surface that did not fail that day is exactly the one that will fail later, on a different host, with nobody remembering the argument.*

**AND THE HOST RULE THAT PROTECTS THE FLEET IS WHAT ARMED IT.** `dontask` is survivable precisely as long as the allow-list is fat, and on Windows workers launched with no `--setting-sources`, so `~/.claude/settings.json`'s grants merged in and carried them for months. **kz-work's standing `--setting-sources project,local` — which exists to keep a foreign user-level Stop hook off fleet sessions — strips those grants**, leaving `state/worker-settings.json`'s entire allow-list: `["Bash(fleet q:*)"]`, one entry. Two correct decisions from two waves that never met, and their intersection is a fleet that cannot run a command. *A defence that works by subtraction removes what was load-bearing somewhere else; ask what the thing you are excluding was also doing.*

**THE S0 CANARY PASSED AND PROVED NOTHING.** `canary-srv` ran `date`, which falls inside what the mode permits, and cleared the 2026-09-08 bring-up. The 2026-07-27 comment says the original `dontask` reasoning *"was never tested against a command outside the allow-list"* — the canary repeated that exact omission on a new host, with a different allow-list, against a comment that names the omission by name. *A canary proves the path it flew; a permission canary that runs only permitted commands is a tautology, and the cheap fix is one command you EXPECT to be denied.*

**MY OWN DEFECT, AND IT IS NOT A HOLE IN THE DOCTRINE — IT IS A DOCTRINE I BROKE.** `knowledge/projects/claude-fleet.md` §Bootstrap hazard, which its own header calls *the live home of the worktree-isolation doctrine*, already says **"Code-touching workers NEVER run in the live install"** and **"at most ONE `bin/fleet.py` writer alive fleet-wide"**. I dispatched a `bin/fleet.py` writer into the live repo. What is genuinely new is only the SECOND ground for the same rule: the section argues it from the bootstrap hazard (worktree edits cannot hot-swap the live CLI and hooks), and this wave adds measurement contamination, which bites even when nothing hot-swaps. Three lanes went into **one shared working directory**. I reached for the 2026-08-05 rule that parallelism is bound only by file-set disjointness, and treated it as the whole test; it is a RELAXATION of a worker-count limit, not a repeal of the worktree rule that predates it. Their edit targets *were* disjoint — which is why the test I chose passed and the wave still lost its measurement. The suite lane's numbers came back `6 failed/4743 passed` on 3.10 and `10 failed/4757` on 3.12 with an ERROR on both, and every one of those anomalies was a sibling lane editing `bin/fleet.py` underneath the run: `conftest.py`'s install-plane hash guard fired correctly on a real mid-run change, and four `test_self_citations`/`test_retired_sid_citations` failures were line-drift from an uncommitted edit landing BETWEEN the two interpreter runs. **A test-suite run is not disjoint from anything — it reads the whole tree, and the disjointness rule is about WRITES while a measuring lane's dependency is on READS.** The rule was satisfied and the measurement was destroyed anyway. *Amendment: a lane whose deliverable is a measurement of the tree runs alone or in its own worktree; disjointness of edit targets is not disjointness of observation.* Two lanes reached the diagnosis independently and `w55-recon-b` cut a throwaway worktree unprompted to get an attributable answer — that instinct is what the next brief should carry by default. Cost: one wave's suite receipt, re-run clean for ~10 minutes.

**A REDIRECT PROTECTS THE STREAM, NOT THE READER.** The gen-0 boot ritual correctly redirects `sup-boot` to a bundle file (class-4 nonce doctrine, never read a secret off a stream tail) and correctly orders `rm`. But step 3 said only *"Read the REST of the bundle file"*, the bundle is ~35KB, and a single `cat` makes the Claude Code harness spill the whole thing — nonce included — to its own durable `tool-results/` file. **So the ritual's `rm` deleted one of two plaintext copies and §5.9's "gitignored is not a retention policy" was violated through a path the doctrine never names.** Found by this incarnation at its own boot and reproduced. *Any tool that persists large output re-creates the artifact the redirect existed to avoid* — the wave-38 `| tail -8` lesson on a different transport. **The successor path was WORSE and nobody had looked:** `_render_successor_task` had no redirect at all and told the successor to read its `NONCE:` off the stream tail — the exact act the gen-0 renderer's own docstring says gen-0 was fixed for, with no file to `rm` afterwards. *One ratified doctrine, two dispatch paths, one repair — check the sibling every time a doctrine is applied to a path.* And `supervisor/JOURNAL.md` records "boot bundle read WHOLE / IN FULL" at ten incarnations, so the defect is lived, not theoretical. Fixed with a sliced-read instruction on both renders plus a forward guard that is a **whitelist of sanctioned forms, not a `cat` ban** (`head -n 99999`, a file-reading tool, and `python -c` all fail it) — because the defect is "read it all at once" and a pin on one spelling passes over every other.

**A FIX WAVE MINTED A LATENT BUG'S DISCOVERY.** Adding the successor redirect exposed that `cmd_sup_handoff_begin` is the **one** `--bg` launch that does not go through `dispatch_bg`, and `dispatch_bg` is the only place `tasks_dir()` is created — so the redirect would have failed the successor's FIRST ACT and stranded the handoff on any home that had never dispatched a worker. Seen red with `state/tasks` genuinely absent before the mkdir landed. *The value of touching an old path is partly what it makes you look at.*

**Self-citation cost, budgeted and under:** +79 lines to `bin/fleet.py`, three numbers at two enumerating sites, **re-pinned BY HAND** (`repin_citations.py` deliberately not used — this campaign measured it producing two corruption classes). Floors predicted +18, actual +19, and the prediction was left as written with the correction recorded rather than edited to match.

**Also this wave:** `origin/server/persistent-fleet` created at `f4aa63f` — 21 commits, including the entire server bring-up, had existed on exactly one disk since 2026-09-08. `docs/SPEC.md` §18 gained the server era (`grep -c keeper docs/SPEC.md` was **0**), and `docs/NEXT-SESSION.md`'s lede no longer opens by declaring the fleet deliberately STOPPED — a month and a machine out of date, and the first thing `CLAUDE.md` sends a reader to. Both were caught with a two-dot range notation whose own command returns 20 while the prose beside it said 21; **a receipt whose command disagrees with its sentence, in the spec of record, found on first review.** Three gates (G-K1, G-K2, G-K4) carried, none ticked, **no fourth raised** — the worker-permission fix has two candidate shapes (default the worker surface to `bypass`, or ship a real `permissions.allow` block in `worker-settings.template.json` and take the `fleet init` migration SPEC §18's fleet-index M2 entry already says is owed), and that is a build slice with a live workaround, not a thing to spend the operator's attention on while three questions wait.

## 2026-08-10 — docket pass: the last four gates ruled, and slice (b) unblocked <a id="2026-08-10-docket-cleared"></a>

**Altai ruled all four open gates in one sitting through the interface; `## Open` is EMPTY and twenty-seven gates now sit under Settled.** **GOALS §8 — APPROVE THE DRAFTED RECONSTRUCTION** (the "prepared text" never existed; the draft was written against the ruling's own three-defect specification, and the operator approves CONTENT — the *paste verbatim / no lane may originate* clauses were the INTERFACE'S safeguard around an artifact it wrongly believed existed, and are withdrawn). **§5 `--yes` — NARROW THE CLAUSE**: the escape exists only where the verb already carries the flag; promoting it globally would collide with the destructive tier's deliberate refusal to honour `--yes`, putting one flag on two surfaces with opposite meanings. **`init --home` — SPLIT `init`, E2 REACHES IT**, using the measured `w47-homes` idiom (flagged tokens in the destructive tuple, bare verb in NO tuple, tier in `VERB_EFFECT_RESIDUAL`) because that form fails SAFE where the naive two-row form fails OPEN — **this unblocks Sequencing §3 slice (b), the last unbuilt slice.** **Slice (c)'s `witness` — VESTIGIAL, DROP IT**: the row derived from it recognised one of three `--fleet-home` spellings its own hooks accept, so a correctly fenced instance reported as UNFENCED, and removing it dissolved three of its gate's findings rather than patching around them. **The standing rule that ruling ratifies is worth more than the row: a build slice may not derive a normative deliverable from a spec word the spec never defines — it files a gate instead**, which is exactly what the `w48-c` lane did against itself. Ruling before (b) is built is the difference between designing the guard in and patching it in.

## 2026-08-10 — wave 53: the stall was unread VERDICTS, and fourteen orphaned gate reports came home <a id="2026-08-10-w53-landings"></a>

**`main` `0d82460` → `5dd045b` and beyond: the launch axis, the liveness axis, and FOURTEEN unlanded gate verdicts landed — nine of them orphaned for up to five waves.** Floors predicted in a commit carrying no results and hit to the digit on BOTH interpreters: `4636` collected after the docs set, `4661 → 4646 passed, 14 skipped, 1 xfailed` after the code axis, rc 0, zero FAILED lines. Push read back from origin byte-identical.

**THE STALL, MEASURED AND NOT EXPLAINED.** Wave 52 dispatched a full slate and its command tier stopped while its lanes kept finishing; the claim heartbeat aged 7423s past the 3600s bound, so wave 53 booted to `seize`. Cause unknown and deliberately not invented. **What is measurable is the shape, and it is a sharper version of wave 46 and wave 37 rather than a repeat.** Those waves lost *finished branches with no reader*. This one lost *finished REPORTS with no reader* — all four w52 lanes completed after the last checkpoint and no supervisor ever opened their verdicts. **A branch with no reader looks like work not done; a verdict with no reader silently converts every downstream landing decision into a guess.** The dispatch brief called it "four finished branches with no reader" and ordered a landing sequence; the real state was four ungraded discharges, and only ONE axis (`w52/live`) had a graded discharge at all. **The landing order is downstream of the verdicts, so reading them first was not preamble — it was the cheapest information in the wave.** Generalisation: *the last artifact a wave produces is the one most likely to be orphaned, because it is produced when the producing tier is closest to its own ceiling.*

**A ZERO CONFLICT COUNT CANNOT TELL "NOTHING COLLIDES" FROM "NOTHING IS THERE".** `w48/gc` presented exactly like the nine genuine orphans — `+1` commit, one added `docs/lanes/w48-gc.md`, **zero conflicts against `main`** — and it was **already landed**: its verdict blob on `main` is byte-identical to the branch's (`29c59ddc…`) while the branch itself is ~26,000 lines stale, so a two-dot diff against it reads like a mass revert. **An add/add of identical content produces no conflict**, so the conflict census — the instrument you would reach for — scores it *free* when it means *redundant*. The separating check is one command per branch: `git cat-file -e main:<path>` before merging. Caught only because ancestry was re-derived at landing time rather than inherited from the brief.

**GATE BRANCHES ARE NOT INTERCHANGEABLE, AND THE MERGE-BASE IS THE WHOLE DIFFERENCE.** A gate branch forks its subject at the sha it gated and carries one verdict commit, so **whether merging it is free or destructive depends entirely on whether the subject has landed since.** Measured in one wave, on eight branches of identical shape: `w52/glive3`, `w52/glive4`, `w52/glaunch3/4/5` brought **the verdict alone** once their subjects were on `main`; `w52/gdfix`, `w52/gdfix2`, `w52/gfork` still based at `64b43c2` would have brought **the verdict PLUS a stale snapshot of work whose branch had since moved.** Nothing in any status output distinguishes them. *Derive `git diff --name-status $(git merge-base main <gate>) <gate>` per gate branch every time; the row order of any table listing them is not the landing order.*

**THE CITATION OFFSET IS NOT CONSTANT, AND THE PRIOR CENSUS SAID IT WAS.** All three w52 code axes touch `bin/fleet.py`, each landing staling the next one's line-number citations. The wave-52 census measured 12 conflicts per pair and characterised them as *"a constant +150 offset, zero semantic content."* The zero-semantic-content half is right; **the constant half is wrong** — in one hunk set, `_identity_abstention_note` moves **−150** and `_print_snapshot_table` moves **+45**. **Two directions at once**, so a re-pin computed as *either side's value plus a delta* is wrong for at least one citation in every merge, and the error is invisible to a sample of two. The correct value is the line the symbol occupies **in the merged tree**, derived there and nowhere else.

**A FIX CITED AS THE REMEDY FOR A SYMPTOM IT DOES NOT TOUCH.** The dispatch brief offered `w50-glaunch` reading `working` for 15.5 hours as evidence for what `w52/live` repairs. **Landed it and re-measured: it does not.** `w52/live` repairs corrupt-registry aborts in the `respawn`/`kill` sweeps — a registry whose *content* is malformed. `w50-glaunch` is a **well-formed** entry (`status=working turns=1 last_activity=2026-08-09T06:21:46Z`) whose *session is gone* and whose status the outcome discriminator never demoted. **Two different defects presenting through one symptom, and the symptom is what got cited.** The record is unsweepable while it reads `working` (`_archive_eligible` will not touch it), so it accumulates exactly like the `dead-suspected` husks that reached 234h — *a signal nobody is obliged to act on stops being a signal*, now with a second status word carrying it. Not remedied: the remedy is a DESTRUCTIVE verb with the operator away.

**A CONSTRUCTION ARGUMENT COVERS THE PINS IT COVERS AND NO OTHERS.** Wave 51 predicted no floor movement on a docs-only landing and missed by 4, so this wave split the claim instead of repeating it: the per-FILE `parametrize` reasoning proves the **count** pins cannot move (only an *added* non-exempt `.md` grows `CHECK_COUNT_DOCS`, and every added file was under `docs/lanes/`, the first `_HISTORICAL_PREFIXES` entry) — and says **nothing** about the **content** pins, which is where nine large verdict documents dense with `fleet <verb>` invocations and `:NNN` citations actually land. Measured separately (645 passed) and labelled a cheap early warning rather than a floor, with the full suite still owed and run. **Both halves held, and the point is that they were different claims needing different evidence.**

**MERGE LOSSLESSNESS HAS A STRONGER PROOF THAN ANY MARKER SCAN, WHEN IT IS AVAILABLE.** Where one side never touched a file, `merged blob sha == branch blob sha` is decisive and costs one `git rev-parse` per file — no digit-masking, no lost-hunk grep whose sentinel shape can betray it. Verified on all 19 landed files. It is available precisely when the change sets are disjoint at file granularity, which is also exactly when a conflict count is least informative.

**And the re-pin tool this campaign built to stop corrupting citations corrupts something else:** gate `w53-gfork` measured it deleting **21,774 CR bytes** — a whole-file CRLF→LF rewrite — while reporting `moved 39, writes 39`, because it reads with universal newlines and writes `newline=""`. On this box `core.autocrlf=true` normalises it into an unchanged blob (`git diff` showed 22/22), so the committed result is untouched **here** — which is exactly why nothing caught it, and exactly why a clone with `autocrlf=false` is where it would bite. *Second corruption class in one tool, both invisible to the instrument watching the tool.*

## 2026-08-09 — wave 51: five landings, every build slice but the operator-gated one, and instruments describing a tree they were not standing in <a id="2026-08-09-w51-landings"></a>

**FIVE LANDINGS, `main` `7b2ff75` → `b57259b`: the LAUNCH BLOCKER (`bin/fleet.py:6134` quotes the `statusLine.command` interpolations — README quickstart step 4 now works), slice (d), slice (e), the `fleet init` operator recipe, and the wave-50 liveness work. Every build slice in Sequencing §3 is now landed except (b), which is blocked on the operator's own gate 3.** Every floor was predicted in writing in a commit carrying no results and hit on BOTH interpreters — 4317→4302/14/1, 4351→4336, 4602→4587, 4636→4621 — except one the supervisor missed and recorded against itself. Every push read back from origin byte-identical.

**THE THEME: AN INSTRUMENT'S ANSWER IS ABOUT THE TREE IT IS STANDING IN, NOT THE TREE IT IS TALKING ABOUT.** Wave 50's theme was instruments caught lying; this is the mechanism behind most of it. Gate `w51-gdtype` reported `CHECK_COUNT_DOCS` **absent from the tree** — it is at `tests/test_doc_claims.py:506`, parametrised over 30 files, but it did not exist *on that gate's own branch*, which predates the launchfix merge that introduced `current_tree_docs()`. The same shape three more times in one wave: a brief told a lane the working-tree digest lived in `BRIEF-TEMPLATE.md` when that was true of `main` and false of the lane's cwd; two parties measured the same declared conflict pair as 25 and 26 and neither could reconcile it; and the dispatch brief quoted a `main` sha that had moved before the brief was read. **Ask which tree an instrument answered about, and make it say so.**

**THE 25-vs-26 CONFLICT CONTROL IS RECONCILED, AND IT IS THE ANCHOR.** `w51-livemerge` closed it: anchored 25, unanchored 26, and **the 26th hit is a prose bullet at `knowledge/lessons.md:994` — inside the lesson documenting that exact failure.** The wave-42 lesson corrupts the instrument it exists to protect, one level deeper than wave 50 found it. **And the campaign shipped that control as a bare `25` with no commit pinned**, while the value is a function of `main` — *a receipt with no pin*, the defect `BRIEF-TEMPLATE.md` diagnoses two paragraphs above where it shipped. **Quote the command, the commit and the number together, or quote none of them.**

**TWO LANDED PINS COULD NOT FAIL, AND THE CENSUS THAT FOUND THEM WAS THE DELIVERABLE.** Slice (e) was defined in the ratified spec by the single word *"pins"*, with three documents disagreeing about which of §7's ten categories it owned. `w51-slicee` answered by planting a mutant against each of the ten rather than reading: **four categories, not ten.** A mutant replacing the append-only homes list with a **whole-file rewrite**, planted in `cmd_homes`, **survived the entire suite byte-identically** — destroying `add,retire,add` history — because the no-rewrite lint's population was two functions and `cmd_homes` was outside it. The "real-list-untouched" pin was a **tautology**: `exists()` around one read under a sandbox, with mutants appending 122 and 117 records to a simulated real list and **all three candidate pins green**. Both landed, both correctly named, neither able to fail. **And the (a)/(e) split that looked like a fifth operator gate was resolved by GRAMMAR** — the disputed parenthetical attaches to *"later slices' plumbing"*, not to *"§7 items"*, which is exactly why none of the ten appears in it.

**A DOCS-ONLY LANDING MOVES THE FLOOR BY CONSTRUCTION.** The supervisor predicted in writing that two documentation commits could not change the test count and **missed by exactly 4**: `CHECK_COUNT_DOCS` is derived as every tracked `*.md` minus a dated-history exemption, with two pins parametrised over it, so `docs/operator/` gaining two files added 2×2 cases (`docs/lanes/` is exempt). **The branch being merged had already written the reason down** — its own discharge had just deleted a bare *"28 files today"* because *"every document added or exempted changes it"* — and the supervisor moved that population 28→30 within the hour while disbelieving the sentence it had just landed.

**A DISCHARGE MUST BE CHECKED WHEREVER THE REPAIRED CLAIM APPEARS, NOT WHERE THE GATE QUOTED IT.** `w51-glaunch2` found the launch-blocker discharge still shipping the retracted census number **328 lines below the paragraph retracting it**, and a file named EXEMPT at two sites while a third site *in the same commit* held it. **Its five questions were all answered soundly and its largest finding was in a sixth nobody asked.** F2's shape inside the discharge of F2 — third instance this campaign of a finding recurring inside its own discharge.

**AN ORACLE CAN BE GREEN CORRECTLY AND STILL BE THE WRONG ORACLE.** The supervisor ordered `w51-livemerge` to run `test_self_citations.py` to fixpoint as the merge's re-pin oracle. That file resolves line numbers in `bin/fleet.py` **about itself only**, so the merge's self-citation cost was genuinely zero and the pin was green correctly — **obeying the instruction and stopping would have shipped 108 stale cross-document citations green.** The lane found the real surface and repaired 15. It also measured that **AST name-containment accepts a re-pin target five lines wrong**: byte-identity of the cited line is the load-bearing check, not containment.

**A PIN WHOSE FAILING PATH PERFORMS THE ACT IT FORBIDS DAMAGES THE MACHINE EVERY TIME IT GOES RED.** `w51-slicee`'s own first RED run exposed that its canary fixture wrote the homes list *before* proving the path was sandboxed — contained by a `USERPROFILE` fence and caught by a guard it added in the same commit. Two further instrument repairs from that lane: **a mutant planter must work on BYTES** (text mode restored a byte-different CRLF file *while printing a matching sha256*), and **a probe must not sit on the surface being repaired** (v1 went silent after the fix, which reads as "fixed").

**THE FENCE DOCTRINE WAS WRONG ABOUT WHY — A CORRECTION OF A CORRECTION.** `BRIEF-TEMPLATE.md` told every lane that wave 48's containment audit came back clean because `~/.claude/fleet-homes.list` did not exist, *"so the lookup population was empty"*. **The population is never empty:** `resolution_population()` appends the legacy install-root home unconditionally (`bin/fleet.py:4621`), and §8's completion is what would remove that term. Driven both ways by `w51-initprep` — sid present + list absent → **the live home**; sid removed → the temp path. The prescribed remedy is unchanged and still correct; only the reason was false, in the one stanza whose entire purpose is teaching a lane to ask *why*. **The supervisor shipped that false paragraph verbatim in four briefs the same day.**

**`fleet init` IS PREPARED AND DELIBERATELY NOT RUN**, and the lane refuted the premise it was handed: **nothing is misrouting today** (all four hooks fall through to an install-root resolution that is correct here because install == home), and **the failure mode is not silent** (an ended session with no outcome record recomputes to `dead-suspected`, which has its own doctor row). Two corrections that would otherwise have reached the operator: **`fleet init` REFUSES a sid-bearing caller while a claim is live** — it must be run from PowerShell, not from a Claude Code session — and **`--statusline` must be omitted**, since what is installed is already fleet's own and the flag would rewrite a machine-global file with an identical value. The `instance-freshness` RED is real this time, unlike wave 46's mtime false alarm: 846 chars against 1002 re-rendered, the whole delta being the four `--fleet-home` additions.

**THE OPERATOR DOCKET WAS LIVING IN A GITIGNORED PLANE.** `state/w48-operator-docket.md`, carried by three consecutive supervisors and named in this wave's dispatch brief as "the set", was untracked — one disposable directory on one machine, in a week this fleet was dead 2.7 days to a power cut. **The gates themselves were never at risk** (`docs/OPERATOR-GATES.md` is tracked and carries all four); it was the *digest* that was perishable, and every brief handed the operator the perishable pointer. Now at `docs/operator/gate-docket.md`. It had also gone **stale at three gates while four were open**.

**Four operator gates carried untouched, none ticked, NO FIFTH ADDED** — including the two places this wave could have opened one and resolved it by measurement instead: slice (e)'s split, and the fence-doctrine defect, filed as a docs edit.

## 2026-08-09 — the fold for waves 44–48, owed for three incarnations: the lesson channel scaled and the process-change channel flatlined <a id="2026-08-09-fold-44-48"></a>

*Written by the `w49-fold` lane. Waves 44–48 ran 2026-08-05T17:35Z → 2026-08-09T01:47Z; entries below are dated by FOLD date, which is this file's convention (`#2026-08-06-w44-landing` folded wave 44 during wave 45).*

**What was actually owed, measured rather than assumed.** Waves 44 and 47 were ALREADY folded (`#2026-08-06-w44-landing`; `#2026-08-09-w47-slice-a` plus `#2026-08-08-docket-and-outage`, which covers wave 46's *outage* but none of wave 46's *content*). **Waves 45, 46 and 48 had no entry at all** — proved by grepping this file for terms that could only appear in them (`2.1.226`, `4217`, `instance-freshness`, `gceil`, `gpins`, `c2572`, `launch rehearsal`, `merge -F`, each returning 0). So "the fold for 44–48 is owed" was **true in substance and wrong in scope**: three waves, not five. The three new entries below are additions, not rewrites.

**THE FINDING, and it is the answer to why lessons recur.** Standing goal 4 says *a process defect becomes a process change, never a repeat incident*, and `playbooks/campaign-template.md` §8 makes amending itself **required to close** a knowledge wave while §3(h) says *"a lessons entry with no process change is rejected."* Measured at `c228d69`: **`knowledge/lessons.md` has 37 commits since 2026-07-25; `knowledge/playbooks/campaign-template.md` has ZERO — it was last touched 2026-07-24 (`f67f875`, still v1.10).** Fifteen supervisor waves recorded their lessons and amended the instrument not once. That is the mechanism behind every "this is the 3rd/4th instance" line in this file.

**But the honest diagnosis is not negligence — the instrument MOVED and nothing said so.** The supervisor era does not run the campaign-template pipeline: it runs waves, lanes and adversarial gates, whose doctrine lives in `docs/lanes/BRIEF-TEMPLATE.md` (created wave 47, amended wave 48), `skills/fleet/supervisor.md` and the specs. **That channel is alive and is being amended.** The template still opens by declaring itself *"a LIVING instrument… every campaign amends it"* while describing a manager/campaign shape that has not run for weeks. *A doctrine file that is stale is bad; a doctrine file that is stale **and still claims to be the live one** routes every future process change into a dead drop.* **PROCESS CHANGE (applied here): template bumped to v1.11**, carrying the waves 44–48 amendments AND a routing header naming `docs/lanes/BRIEF-TEMPLATE.md` as the live instrument for lane-era work. **Still owed and NOT mine to apply** (this lane is fenced to `knowledge/**`): the BRIEF-TEMPLATE safety-stanza repair named in `#2026-08-09-w48-fences`.

**A second structural reason lessons recur, visible only from doing this fold: this file cannot be searched by the reader who needs it.** Entries are single paragraphs of 400–900 words with the finding buried mid-sentence, and INDEX lines have grown into the same shape — so "has this been recorded?" costs a full read, and the cheap answer is to record it again. Three of the items this fold was told to look for turned out to be *sharpenings* of lines already here rather than new facts, and I could only establish that by grep on distinctive tokens. **PROCESS CHANGE: every new entry leads with a bolded claim sentence that is greppable on its own terms, and INDEX lines front-load the finding.** Applied below; not retrofitted, because rewriting dated records to fix a lookup problem is the failure this file already names.

---

## 2026-08-09 — wave 48: the safety fence every lane ran under was made of nothing, and a gate's coverage is a property you must measure <a id="2026-08-09-w48-fences"></a>

*Wave 48 = `inc-20260808T232242Z-ae09`, 2026-08-08T23:22Z → 2026-08-09T01:47Z. Landings `5ebba74` (pin tier report), `7c3e48b` (install rehearsal, gated + discharged), `8e01d83` (root `CLAUDE.md`), `44f056e` (BRIEF-TEMPLATE safety stanza), `55091c3` (journal). **Four floor runs, four predictions written before the run, four exact hits on both interpreters** — 4154 collected each time, and derived by `--collect-only` on both trees rather than by arithmetic on a diff.*

**`FLEET_HOME` IS NOT A FENCE INSIDE A FLEET-LAUNCHED SESSION.** Multi-fleet §5 step 2's sid→home lookup `return`s on a hit, so it outranks step 3's validated env. Measured, three runs, one discriminator: env set + sid present → the live home (**ignored**); env set + sid **removed** → the temp dir. Every brief on this machine had been fencing lanes with that variable, including the wave's own live pin lane, which had to be steered mid-flight. **This is ratified behaviour working exactly as specified — the code is right and the BRIEFING DOCTRINE was wrong, which is the more dangerous shape, because nothing tests a brief.**

**AND THE CORRECTED DOCTRINE IS ITSELF INCOMPLETE — measured by the slice-(c) gate one wave later, and this is the part still unfixed.** The repair landed at `44f056e` says *pass `--fleet-home <temp>` explicitly*. That flag **cannot name a home you are about to `init`**: §5 step 1 validates the target is an *initialized* home, and `home_is_initialized`'s own docstring records as a ratified, deliberately-pinned consequence that **`fleet init` does not create `state/fleet.json`** (the registry appears on the first spawn) — so `fleet home --fleet-home <fresh temp>` refuses `not_initialized`. The gate's actual fence was **removing `CLAUDE_CODE_SESSION_ID` from the child environment**, gated on `fleet home` run the identical way. *The shipped stanza names the flag and not the mechanism, so it fails on precisely the case a safety fence exists for — a brand-new throwaway home.* **PROCESS CHANGE, named and NOT applied here (fenced out of `docs/lanes/`): the BRIEF-TEMPLATE safety stanza needs (a) "unset `CLAUDE_CODE_SESSION_ID` in the child env" as the primary fence, (b) the not-yet-initialized carve-out spelled out, (c) the existing `fleet home`-run-identically gate kept as the verification.**

**A CONTAINMENT AUDIT CAN COME BACK CLEAN FOR THE WRONG REASON. ASK *WHY* CONTAINED, NEVER MERELY *WHETHER*.** Nothing reached the live home — but only because `~/.claude/fleet-homes.list` **does not exist**, so the lookup population is empty and the live home is never a candidate, while that lane's own sid *was* in the live registry. One `fleet homes --add` and the tier spawns into the live fleet and stamps the real `pin-pass.json`. **The temp-home safety of every lane on this machine rests on the accident that nobody has created the homes list — and slice (b), `init --home`, is precisely the thing that creates it.** Three independent routes reached that list in one wave (the pin audit, the launch gate's MAJOR-1, the E2 classification work). The lane volunteered this rather than banking the clean verdict, which is the whole reason it is known.

**A GATE'S COVERAGE IS A PROPERTY TO BE MEASURED, NOT ASSUMED — and it was, by withholding.** The supervisor found a garbled phrase the branch added (*"inside a session fleet launched"*), deliberately did not tell the gate, and the gate returned **seven findings with that not among them**. *So a gate of this shape attacks semantics rigorously and does not proofread — and neither does the suite* (`test_doc_claims.py` pins verbs, flags and counts against the parser, never prose). **The discharge then found the class is bigger than grammar:** three more sentences were *unquantified quantities wearing the clothes of measurements* — *"often works"* (a frequency claim), *"for a wave"* (a duration claim), and an unpinned link count (a claim about an unnamed tree). **PROCESS CHANGE: a proofreading pass over a measurement-graded document is not spellcheck — it is where unquantified intensifiers get caught, and nothing in this harness looks for them; order it explicitly in every doc-lane discharge brief.**

**`fleet send` CAN PRINT `fork-steered` WHILE THE WORKER SILENTLY REDOES ITS PREVIOUS TASK.** Fork-steer rewrites the task file, then dispatches a prompt (`Read <task file> and follow it exactly`) that is **byte-identical** to one the forked session has already answered — and the fork carries the full transcript, so it can answer without re-reading. Forensics from the surviving transcript: two turns, byte-identical 107-char prompts, and the fork's first act is not a `Read`. **Fleet's mailbox delivery is CORRECT** — a probe preserving the home across the `send` found the steer body on disk **3/3**, absent before the send **0/3**. *Nothing forces the fork to look.* **1 failure in 5 samples** at 2.1.226. Crucially the lane refused to call it new: it has **no 2.1.222 sample**, and *the likeliest reading is a latent long-standing defect whose prior green runs were samples that got lucky* — a ~20% failure mode needs only a handful of single-run greens to stay invisible. **PROCESS CHANGE, named by the lane: make the pin's steer token unique per run, so a stale answer can never coincidentally match; and fix the dispatch prompt (inline the manager message) rather than weakening the assertion.** The supervisor acted on it the same turn it learned it — **dispatching the next discharge by `respawn`, not `send`**, because using the verb under suspicion to deliver the fix for the wave's other defects was a free risk. Related but NOT the same as `#2026-07-31-w35-brief-store`'s fork-steer finding: that one is about the task file being *destroyed*; this one is about a correctly-written file never being *read*.

**THE SKIP COUNT WAS THE HEADLINE AND IT CAME BACK ZERO.** The native pin tier ran with **zero skips on both interpreters** (py3.13 `1 failed, 5 passed`; py3.10 `6 passed`) against an ungated baseline of `6 skipped`; the suite's 14 skips were all enumerated and **6 of them ARE this tier**; the lane went looking for a second gate beyond `FLEET_LIVE=1` and established there is none. **No CLI drift 2.1.222 → 2.1.226.** *A green suite certifies nothing about the native contract unless the skips are proved zero* — first time this campaign the number was proved rather than assumed. **The stamp was HELD deliberately**: five of six pins justifying the version is not a reason to stamp when the sixth is a RED against shipped code, so `pin-version` keeps FAILing, correctly.

**A MUTANT CAN SURVIVE A FULL 4217-TEST SUITE. THREE DID — AND THE LANE'S OWN TABLE WAS TRUTHFUL.** The slice-(c) gate re-planted the lane's six mutants formulated from the code rather than copied from its report: **6/6 died.** Its own five: **X2, X4 and X5 SURVIVED the full floor** (`4217 passed, 14 skipped, 1 xfailed`). **All three survivors are the gate's own and all three are the same subject — the `home-witness` row the lane added last.** So the standing lesson (*a self-reported fault-injection table is a claim about the mutants its author thought of*) arrives in a sharper form: **the table can be entirely truthful and still incomplete, and the gap clusters on whatever the author built most recently.** **PROCESS CHANGE, the gate's own protocol: a mutant that goes GREEN on a subset is not a survivor until the full suite has had its chance** — all three were escalated to the full floor before being reported, against a clean full floor measured first.

**THE GATE DECLINED A SEVERITY ITS BRIEF HAD PRE-COMMITTED, AND SAID SO.** The brief pre-committed BLOCKING on one attack; the gate graded it CLEARED with the measurement, and wrote: *"Manufacturing a BLOCKING finding to match the a3 precedent would have been this gate's failure mode."* This is `#2026-07-31-w37-gates`'s *a brief that pre-commits a severity is choosing the verdict* firing again, and the campaign now supplies the pressure — after a3's gate found a live fail-open, "the standing expectation is a live fail-open" went into a later brief as an expectation. **PROCESS CHANGE: a gate brief may name what to attack and may never name what grade the result deserves.**

**THREE MORE INSTRUMENT FAILURES, ALL CAUGHT BY CONTROLS BEFORE THEY BECAME FINDINGS.** (1) `git merge-tree --write-tree` parsed as a rev again at git 2.34.1 — it would have reported "0 conflicts" by measuring nothing; caught because a known-conflicting control ran FIRST. (2) `cmd /c` mangled the quoting in an executor so the **control failed with the identical error as the subject** — which is how the gate knew the instrument had failed rather than the subject; redone through a real shell script. (3) **A 10-minute tool timeout killed a planter mid-run and stranded a mutant on disk**, caught immediately by `git status` plus a 233-file sha256 manifest and restored before anything else ran. *A planter needs an on-disk recovery guard, because the thing that kills it may not be its own code.*

**`git merge -F -` DOES NOT READ STDIN AT THIS GIT VERSION** — it dies `could not read file '-'`, rc 129. A real file is the only form that works, and both of this wave's merge messages went through `-F <file>`, were read back and span-checked, and never touched a shell string. This is the mechanical discharge of `#2026-08-09-w47-slice-a`'s REFUSAL-CLASS-5 correction: *the hazard is the SHELL, not the verb,* and a message file has no shell in it at all.

**Smaller, each of which cost someone real time:** the install rehearsal found three launch blockers by following the published quickstart exactly — a bare `fleet init` configures whatever fleet is already on `PATH` rather than the clone you stand in (the 2026-07-29 incident class, reachable from the docs), the quickstart's `PATH` step is a **comment rather than a command** so a clean machine dies at step 2, and `--fleet-home` — the one override that outranks the sid lookup — appeared in **no user-facing doc**. **`fleet init` does not produce an "initialized home"**, and the refusal three feet away uses that exact word: deliberate and pinned, but undocumented and R2-shaped (names a definition and a spec path, no remedy). **launch-gap 8 was FALSE rather than merely unlisted**, root-caused to a stale root `CLAUDE.md` and fixed at `8e01d83`; the gate found the bookkeeping omission but did not re-measure the gap itself. **A true receipt the pin cannot parse**: a pasted `git log --format='%h %ad %s'` whose subject contained the words *"fleet is"* tripped `test_no_doc_invents_a_fleet_verb`, resolved by dropping `%s` and re-running so the pasted output is still exactly what the command prints — **not** by weakening the detector. And still live at `c228d69`, INFO: **`user_settings_path`'s docstring calls `~/.claude/settings.json` "the ONLY file outside FLEET_HOME that fleet ever writes" while `homes_list_path` calls itself "The SECOND file"** — the second is correct; exactly the stale absolute a doc lane reads and promotes into user-facing prose, which is what that wave's MAJOR-1 was.

**THE PEEK PROBLEM — and this fold is where it surfaced.** Wave 48 stood down while its slice-(c) gate was still running and recorded what it had seen **by peek**, hedging honestly that a peek is ephemeral. Two of those peek-derived claims are contradicted by the committed verdict (`docs/lanes/w48-gc.md` on `w48/gc`): the unquoted `{fleet_py}` was written into the successor queue as *"a real breakage… must be discharged before (c) lands"* and is graded **MINOR, explicitly "pre-existing… not a blocker"**, with the verdict answering **"No BLOCKING finding"** and GATING on three MAJORs the peeks never saw; and the mutant survival was attributed to the lane's table being incomplete when the verdict says that table is **truthful, 6/6**. **A peek is a gate's working state, not its verdict, and a mid-run alarm is systematically louder than the final grade.** **PROCESS CHANGE: a successor queue may cite a still-running gate only by ARTIFACT PATH and status ("unread, verdict commits to `docs/lanes/<name>.md`"), never by transcribing peeked findings as findings.** The `docs/lanes/` convention already made the verdict durable without its supervisor — which is exactly why the correction was available to be made at all.

---

## 2026-08-09 — wave 46: a `fleet result` is not a verdict, and the two documents are written at different fidelities <a id="2026-08-09-w46-fidelity"></a>

*Wave 46 = `inc-20260806T005348Z-e7a3`. It booted 2026-08-06T00:53Z, dispatched four lanes, wrote its last checkpoint at 01:09Z and died — sixteen working minutes, then a 2.7-day silence ended by seizure. The cause was the operator's PC being off and is already recorded at `#2026-08-08-docket-and-outage`; what follows is the wave's own content, which was never folded. All four lanes finished their work on branches and nothing was lost.*

**`fleet result <gate>` IS A LANE-AUTHORED SUMMARY. THE VERDICT IS AN ARTIFACT. THEY ARE NOT THE SAME DOCUMENT, AND THE DIFFERENCE WAS 12×.** The dispatching brief stated *"the verdict content is lossless in `fleet result w45-ga2`; read it there."* Measured false: **`fleet result` returned 3,117 bytes; the verdict file was 38,815 bytes.** Obeying the brief would have landed multi-fleet slice a2 without ever seeing the verdict's §6 — **nine advisory findings, three of them proven by mutants that left the full suite byte-identically green.** *A result is not a verdict, and a lane that writes both writes them at different fidelities.* The 3-times-recorded lesson (*a finished result with no reader is work not done*) had always been about nobody reading; this is the sharper case where someone reads **the wrong artifact** and it looks like diligence.

**The cause was structural and became the `docs/lanes/` convention.** The gate wrote its verdict into its own detached worktree's `state/`, which is gitignored and per-worktree, so it existed on exactly one disposable disk. This supervisor hand-copied it to the main tree; the next wave found that **`fleet result` had since become unavailable anyway**, because the lane's registry record was archived and the result went with it. **Archiving a lane destroys its result** — so the durable artifact must be a FILE that rides a commit, and the result may only ever be an index into it. Built one wave later as `docs/lanes/`.

**A DOCTOR ROW THAT COMPARES TIMESTAMPS INSTEAD OF CONTENT IS A FALSE-ALARM GENERATOR, AND `instance-freshness` HAD BEEN ONE FOR SIX DAYS.** The row compares **mtimes**: the template last changed in git at `94a549b` (2026-07-31) and the instance was rendered 2026-07-30, so the row was *literally true*. But that commit's entire change was `{{FLEET_HOME}}` → `{{FLEET_INSTALL}}` across four hook commands, and on the legacy single-fleet layout `fleet_install` defaults to `fleet_home` — **so both render the same string.** Proved by RENDERING rather than by reasoning: the rendered template and the live instance came back **sha256-identical, 846 chars**. The remedy the row names (`fleet init`) would have written the same bytes. The check's own docstring names the cost — *"flagging them would train the operator to ignore this line"* — and the blind spot was acknowledged in one direction (an mtime-newer hand-edited instance reads PASS) and **not** in the other. **PROCESS CHANGE, filed and still UNBUILT across three waves: compare the RENDERED template against the instance, not mtimes** — strictly stronger, goes green here and still catches real drift. **And wave 48 found the wrinkle that makes this urgent rather than cosmetic: on a branch that genuinely changes the template the same row fires CORRECTLY. One row, two meanings depending on the tree** — which is exactly the condition under which somebody "fixes" the wrong half.

**THE PIN WRITTEN TO CATCH A SUPERSEDED BAND DID NOT COVER THE DOCUMENT THAT BINDS THE READER.** Shipped `band_thresholds('supervisor')` → `(350000, 400000)` while `supervisor/GOALS.md` still read *"Context band (150–200k, supervisor AND workers)"* — and `test_no_superseded_band_is_stated_as_current` passed **4/4** the whole time, because its `SURFACES` tuple lists two skill files and not GOALS.md, **the file the boot ritual loads FIRST into every incarnation.** The omission was *defensible* (an operator-owned file would go RED with nobody permitted to repair it) and **undocumented**, which is the part that made it a hole rather than a decision. *A pin's `SURFACES` tuple is a coverage claim; enumerate what it excludes, in the pin, or the exclusion becomes invisible.* Ruled and closed two waves later — GOALS.md joins `SURFACES` once its lines are true.

**A JUDGEMENT CALL RECORDED SO A SUCCESSOR COULD DISAGREE, which is the habit worth copying.** Reading the 38,815-byte verdict in full put this supervisor at ~118k against a brief that said *"dispatch in your first 30k"*. It did it anyway, wrote down that it had, and gave the grounds: the brief's instruction rested on a false premise about the result's fidelity, and the three lanes' briefs were materially different documents for having read §6. **Naming an overrun and its reason costs one paragraph and is the only thing that lets the next body price the trade differently.**

---

## 2026-08-09 — wave 45: the ceiling went live, and two pins were measured green over holes they could not reach <a id="2026-08-09-w45-ceiling-pins"></a>

*Wave 45 = `inc-20260805T193255Z-4ee5`, 2026-08-05T19:36Z → 22:24Z, ended by operator stand-down. Landings `238a4778` (the 400k ceiling raise, floors 3921/14/1 = 3936 predicted and hit exactly on both interpreters) and `1be5beac` (claim-nonce §8 restatement under a self-audit gate). Three adversarial gates ran — `w45-gceil`, `w45-gpins`, `w45-ga2` — and **two of the three found a blocking defect that the full suite could not see.* The raise is live: `sup-context` rendered 168,251 BELOW-BAND against 350,000–400,000 at the shipped tree.*

**A CITATION REPOINT MOVED A RANGE'S START AND LEFT ITS END BEHIND, PRODUCING A BACKWARDS RANGE THAT NO PIN COULD SEE.** The ceiling branch repointed 31 citations; one of them, `cmd_respawn:7341-7343`, became **`7443-7343`** — start moved `+102` like every neighbour in the hunk, end untouched. **Four independent reasons nothing caught it, and all four are structural rather than luck:** the citation regex spells the range end `(?:-\d+)?` — **non-capturing and unvalidated**, so the scan returns the start only; the pin's own docstring *says* it takes the range's first number; the unclassified-number guard matches `:(\d+)` and **`-7343` carries no colon**, so it is not even a token to that test — doubly blind; and **the lane's own control could not see it either**, because the control was *"the count of `:NNNN` tokens is unchanged at 77"* and `-7343` is not that shape. That last one is `#2026-08-05-w42-landing`'s *a control must include the SHAPE of the data, not merely an instance* firing again, one wave later, against the author who had it available. The gate proved the blindness with a three-value mutant: **shipped (backwards), absurd (`7443-9999`) and correct (`7443-7445`) all returned an identical `18 passed`.** *A pin gives the same green to the defect, to nonsense, and to the fix.* **PROCESS CHANGE, built and landed in `w45/pin-hardening`: `_NAMED_CITATION_RE` captures the range end and asserts `end >= start` and that the end resolves inside the same function — with `7443-9999` kept as the seed, because a range check that cannot fail proves nothing.**

**A DATED-LINE CARVE-OUT SWALLOWED AN ENTIRE SURFACE, AND THE PARAMETRISED GREEN READ AS COVERAGE OF A FILE THAT HAD NONE.** The pins lane's new "superseded band edge stated undated" check exempts any line carrying a `\d{4}-\d{2}-\d{2}` **anywhere on it**. Census of every band-bearing line on its two surfaces: on `skills/fleet/supervisor.md` some lines are dated and some are not; **on `skills/fleet/SKILL.md` every single one is dated.** So on that file the carve-out exempts everything and the param is unreachable — the gate's mutant survived **713 tests**, while a control planting the same numbers onto an *undated* line reddened immediately, proving the param itself works. The lane had disclosed a residual honestly (*"a revert that also writes a date onto its own line still escapes this, and that is a deliberate act rather than the drift this catches"*) — **true of `supervisor.md`, false of `SKILL.md`, where no deliberate act is required because the date is already there.** *An exemption is a coverage hole until you census the surfaces it applies to and count what it removes; if it removes every candidate line on a surface, that surface's green is false.* **PROCESS CHANGE: any pin that ships a carve-out ships the census of what the carve-out exempts, per surface, in the test's own docstring.**

**A LANE CAUGHT THAT A GATE VERDICT'S QUOTED MUTANT WAS WEAKER THAN THE VERDICT'S OWN PROSE — AND PLANTED THE WEAK ONE.** Re-planting from a verdict, the pins lane noticed the quoted plant did not exercise as much as the surrounding paragraph claimed; it planted the honest weaker one **and** pinned the fuller property as a second assertion. *A verdict is evidence, not scripture: re-plant from the code and treat the verdict's quoted patch as one sample of the class it names.*

**Three lanes ran in parallel and disjointness was by CONSTRUCTION rather than by hope** — each brief fenced the other lanes' files **by name**, and the one real collision risk found in advance was closed by relocation (two new pins that would naturally have landed in a file another lane held were shipped in their own file instead). The cross-lane hazard was written into the brief explicitly: *a real defect a lane's new pins find in a file another lane holds is report-and-stop, not fix.*

**Also worth keeping.** The ceiling lane's own report scored its brief wrong **six** ways, the largest being that **no worker-band constant existed at all** — §11.4's worker arm was `[UNBUILT]` and the branch did not build it — and that self-citation cost it **three fixpoint passes over 31 numbers**. It disclosed a deliberate divergence rather than hiding it (indeterminate identity refuses at the supervisor ceiling while measurement reports the worker band; strict in both directions, and pinned). And the wave ended on an **operator stand-down taken cleanly**: the supervisor wrote its successor's campaign brief into the journal and released rather than dying, *because a released claim reads unambiguously at the next boot — which is exactly what distinguishes an authorised stop from an outage.*

---

## 2026-08-09 — wave 47: multi-fleet slice (a) complete, four landings, four exact floors, and three instrument failures with one cause <a id="2026-08-09-w47-slice-a"></a>

**Slice (a) is done end to end (a1+a2+a3), plus E5 and a report-durability build — five merges, and every floor predicted IN WRITING before running and hit EXACTLY on both interpreters: 3974, 4052, 4119, 4119, 4134.** Landings: `663acb9` pin-hardening (ga2's B2), `74a5abf` a2 + its B1 §5 divergence note, `c3ba464` a3 the wrong-home guard, `9c15680` E5, `e5889fe` lane-report durability. **The mission's "three landings" were two merges** — `w46/s5-note` was branched off a2's tip, so a2 was its ancestor and landing the note landed the code; that turned out to be the *better* shape, because the a2 gate was GATING on B1+B2 and the note IS B1, so a2 could not exist on main for even one commit without its divergence record.

**THE SECURITY FINDING: the wrong-home guard FAILED OPEN on the empty string.** `verb_effect_tier` decided a residual flag was set by **truthiness**; `cmd_sup_decision` decided it was writing by **presence**. The two agree on every value but `""`. Measured through `main()` on an armed machine: `fleet sup-decision --answer ''` classified **ordinary**, passed the guard, and read-modify-wrote a **foreign** home's decision file, while `--answer yes` was correctly refused. `fleet sup-decision --answer "$UNSET"` is the field shape. **Why 4061 tests could not see it, and this is the transferable half: every test of that classification built its `argparse.Namespace` BY HAND rather than taking one from `build_parser()`, and the parametrised fail-safe test happened to use two truthy values.** *A test that hand-constructs the input of the thing it tests is testing the function, not the program* — the whole defect lived in the seam between parser and classifier, which no hand-built namespace can cross. Same family as ga2's B2 one slice earlier (a pin green because it drove `parse_args`, a layer the flag no longer reached), arriving from the opposite direction. Fixed with `not in (None, False)` — and note the naive `is not None` breaks `doctor`, whose `--repair`/`--clear` are `store_true` defaulting to `False`.

**THREE INSTRUMENT FAILURES IN ONE WAVE, ALL THE SAME CAUSE, ALL CAUGHT.** (1) The gate's mutant planter **refused** a patch (*"matched 0 times"*) and its driver ran the suite anyway — producing a clean floor that is byte-indistinguishable from a green mutant run; it was one paragraph from a fabricated advisory before round 2 re-planted properly → RED. (2) The durability lane's own detector asked `git check-ignore` about `"<path>\r"` because `subprocess` text mode appends a carriage return on Windows, got "nothing ignored", and would have made **every verdict green and worthless**; fixed with NUL-delimited bytes. (3) The supervisor passed a backtick-dense merge message through a **double-quoted** shell string and executed three spans, leaving three sentences missing the very things they were about. **CRLF-vs-`\n` and shell quoting are the same wound: an instrument that fails SILENTLY converts "I measured nothing" into "I measured zero".** The standing remedy is now cheap and universal — **a plant driver must assert its own patch applied before it runs anything** (`occurrences == 1`, abort otherwise); the supervisor used exactly that when re-planting the B1 mutant by hand.

**REFUSAL CLASS 5 HAS A SECOND SURFACE, AND THE RECORD NAMED THE WRONG ONE.** The 2026-07-31 entry frames backtick execution as a *checkpoint* hazard, so `@file` for `sup-checkpoint` reads as the whole fix. It is not: **the rule is about the SHELL, not the verb.** A merge commit message is the second-largest block of backtick-dense prose a supervisor emits and it goes through the same double-quoted string. Caught only by reading the message back — the exit code was 0 and the merge tree was correct. Use a quoted heredoc (`-F -` for `git commit`, a file for `git merge`, which does **not** read `-F -` from stdin).

**REPORTS DIED IN GITIGNORED PER-WORKTREE `state/` — THREE TIMES IN ONE WAVE — SO IT WAS BUILT OUT.** The `w44-ceil` lane's report §8 carried the prepared `supervisor/GOALS.md` replacement text and is **unrecoverable**, blocking an operator ruling; slice a3's self-report was destroyed when its registry record was archived, forcing its gate to audit the branch from the code; the a2 gate's 38,815-byte verdict survived *only* because a supervisor hand-copied it. Now `docs/lanes/`: a lane's report is **committed on its own branch** and arrives at main with the merge, so durability is a consequence of work the lane already does. **The lane corrected the supervisor's causal claim, and the correction is the load-bearing part:** the machine preamble already renders an ABSOLUTE journal path — reports died because the *hand-authored deliverables line* named a RELATIVE one (**55 briefs relative, 84 absolute**). So the obvious repair, "make every brief absolute", leaves the file just as gitignored and just as uncommitted; the pin normalises `$FLEET_HOME` prefixes so that half-fix cannot pass. *When a defect has an obvious repair, check whether the repair addresses the cause or only the symptom you happened to notice.* The disposable journal is kept **deliberately** — it is the crash-survivable partial record when a lane dies before writing its report. **Still true and stated as a decision, not an oversight: deleting a lane's branch destroys its report.**

**A GATE THAT RESTORES ITS PLANTS PERFECTLY IS BLIND TO EVERYTHING THAT ONLY BREAKS WHEN LINES MOVE.** The a3 gate restored every mutant byte-identically and therefore never shifted a line — so it could not see that inserting 37 lines into `bin/fleet.py` reddens **7 tests** across `test_self_citations.py`/`test_retired_sid_citations.py` and costs **32 re-pinned numbers**. The discharge lane found it on its first full floor (`7 failed, 4059 passed`) and repaired it by difflib-mapping each number against the base and content-verifying per rewrite — **never by adding a constant**, which produces a citation *generator* rather than a citation fixer (wave 35). **Budget a self-citation pass on every `bin/fleet.py` edit.** The reverse also got measured: a2's landing owed **zero** fixpoint passes, because main had not moved `bin/fleet.py` in the interim — *the fixpoint cost is a function of what BOTH sides moved.*

**A SAFEGUARD AROUND A MISSING ARTIFACT IS NOT A RULING.** The GOALS ruling reached the supervisor fused with a relay clause ordering the text pasted *verbatim* from a file that does not exist, plus *"no lane may originate GOALS.md content"* — making the lane unexecutable. The supervisor refused to improvise and filed a gate; the interface then disclosed that **both clauses were its own over-specification, not the operator's decision.** *A docket relay and an operator's answer are two documents with two authorities, and they arrive as one — when a binding clause blocks the work it authorises, ask which layer wrote it before assuming it is ratified.* The replacement is now drafted and PARKED at `docs/proposals/2026-08-09-goals-band-section-replacement.md` with every number re-derived from `band_thresholds` and every build-state claim re-derived by grep, because inheriting any of it would repeat the exact `[UNBUILT]` class the replacement exists to fix. **Measured while drafting: the supervisor arm IS built (5 call sites, 4 verbs, both respawn sites armed only with `--task`); the worker arm is NOT** — its constants have one consumer, `band_thresholds`, which merely reports, and the ceiling predicate exempts a worker by identity before any threshold is compared. `supervisor/GOALS.md` is untouched and `SURFACES` unchanged: adding the surface before the text would redden a pin nobody but the operator may repair.

**A RECEIPT THAT QUOTES ITS OWN SEARCH STRING POISONS THAT SEARCH.** The gate recording the lost GOALS text quoted `git log --all -S"Context band (350"` as proof it returned EMPTY. That command now returns the gate commit itself — **the search finds only its own obituary.** Substance unchanged, instrument spent. Same family as the pin220 lesson that a dated stamp is not a current-state stamp.

**Also measured, small and reusable:** floors were derived compositionally rather than re-measured, twice, and both held — a2's from the gate's tip figure plus the pins delta (4014+38=4052, agreeing with an independent 3974−76+154), and a3's from the gate's own throwaway-clone merge plus the discharge's `--collect-only` delta (4114+5=4119). **A gate that merges into main in a disposable clone hands the supervisor a landing number the branch cannot produce**, and it retired the citation-fixpoint question before the merge existed. The three-arg `merge-tree` control was re-run before **every** conflict measurement and returned 26 each time; every measured zero came with a non-empty byte count beside it. And the E5 lane found that **the §5 pins parse the raw destructive-row line for backticked verb tokens**, so naming `` `peek` `` inside the row — the natural phrasing of the ruled ground — flips two pins RED; the verb contrast lives below the table instead.

**One defect FILED, NOT FIXED:** `fleet_lock` catches only `FileExistsError`, but Windows returns `PermissionError` (EACCES) for a **delete-pending** file, so a contender racing the holder's `unlink()` escapes the retry loop entirely and callers written against `FleetLockTimeout` get a raw OSError. Found by a floor run under load, proved not-a-regression by extracting and hashing the function on both sides of the merge (`c54382c4…` identical), 12/12 green in isolation. Core-safety code earns its own gated lane; brief ready at `state/tasks/lens/w47-locksafe.md`.

**CORRECTED 2026-08-09 (the waves 44–48 fold) — that defect was FIXED in this same wave, and the mechanism stated above is WRONG.** The dated paragraph is deliberately left as written. Two corrections, both measured: (1) **It landed at `f402895`** (*"merge(w47): fleet_lock no longer lets a Windows EACCES escape the retry loop"*), the seventh landing of wave 47, floor 4154 predicted and hit on both interpreters — so *"filed, not fixed"* was true when written and false a few hours later. (2) **The mechanism above — a contender racing the holder's `unlink()` — is not how the window opens.** The lane that fixed it measured that `unlink()` alone does **not** usually produce delete-pending on Win10 1809+, because `DeleteFileW` prefers POSIX-semantics delete and the name simply vanishes; the window requires **a third party holding a handle opened without `FILE_SHARE_DELETE`**, which explains the flake's rarity far better than the "under load" hypothesis in the brief that commissioned it. It also measured **`winerror=None`** on that exception (it surfaces through the CRT `_wopen` path, not `CreateFileW`), so **a fix keyed on `e.winerror == 5` could never have fired** — the errno in the original brief was right and the mechanism was wrong, and only the second would have misled the fix. **Residual, disclosed and NOT closed:** `unlink()` on a delete-pending name raises `PermissionError` too, so the stale-break arm carries the same escape shape and is now the only uncaught `PermissionError` on the acquisition path. Full measurement: `docs/lanes/w47-locksafe.md`.

## 2026-08-08 — docket pass: three gates ruled, the docket cleared, and a 2.7-day outage with a known cause <a id="2026-08-08-docket-and-outage"></a>

**Three gates ruled by Altai in one sitting through the interface; `## Open` is now EMPTY.** **E5** — the §5 destructive row's `doctor --repair` justification is **re-grounded on the rename**: `--repair` renames the target home's `state/fleet.json` aside, destroying a file the operator may want to inspect, and nothing un-renames it, while `peek` only reads — the same effect-grounded discriminator already ratified for E1/E2. **E2/homes — SPLIT THE VERB**: reading `fleet homes` stays ORDINARY, `--add`/`--retire` become DESTRUCTIVE. Two grounds, both operator-named: `--add` irreversibly appends to the machine-global `fleet-homes.list` (only the fold reverses), which is the E2 ground exactly; and ORDINARY meant a read-only `/fleet:*` grant of `Bash(fleet homes)` *reached* `--add` — the 2026-07-09 security class returning, where a read-only slash surface could reach `kill`/`clean` because the grant covered the whole CLI. Splitting closes the hole without pushing read-only surfaces toward destructive grants. **GOALS band — apply the prepared §8 replacement IN FULL and ADD `supervisor/GOALS.md` to the pin's `SURFACES` tuple.** Full, not numbers-only, because the same block also tags the §11.3 dispatch refusal `[UNBUILT]` when it shipped at `c6fde34`/`d969de3` and conflates the built supervisor arm with the genuinely unbuilt worker arm — *a numbers-only edit leaves false claims standing under a pin that then blesses them.* The pin objection (operator-owned file ⇒ RED with nobody permitted to fix) was **answered, not waived**: after correction it is green and only re-reddens when a future ruling supersedes the band, at which point the required fix is an operator edit — which is who should make it. Binding on the executing lane: paste the prepared §8 text verbatim; **no lane may originate GOALS.md content.**

**THE OUTAGE, CAUSE KNOWN, NO DEFECT.** Wave 46 booted 2026-08-06T00:53Z, dispatched three lanes, and the command tier died; wave 47 booted 2026-08-08 against a **232,044s stale heartbeat**. Cause, from the operator: **the PC was off.** All three lanes had FINISHED and every artifact survived on its branch — durable workers outlive the machine, the command tier does not, and *the operator relaunching the interface IS the restart trigger* (2026-07-27's ruling, working as designed for the second measured time). **The lesson is not about the gap, it is about what the gap made visible:** five `dead-suspected` husks had accumulated to 234h because `_archive_eligible` gate 2 admits only `{idle, dead, interrupted}` and gate 4 reserves "no outcome record" for human investigation — correct by design, **but the verdict has no clearing ritual, so the investigate queue only ever grows.** Same shape as *a permanently-red doctor is a disabled doctor*: a signal nobody is obliged to act on stops being a signal. Cleared by operator instruction with `kill --yes` then archive — never `clean`, so the journals survive.

## 2026-08-06 — wave 44: three landings under gates, and the bound that failed on the conjunct nobody was watching <a id="2026-08-06-w44-landing"></a>

**The staged-handoff pattern paid in full: both lanes dispatched in the first ~92k from the predecessor's staged briefs and pre-cut worktrees, and every prediction hit.** Landings `5c2e5ac` (E2/homes docket line), `c5e84c9` (claim-nonce `:337` restatement), `d84a3c1` (§5-criterion ruling + `w42/mf5-verbs`, one landing under an adversarial gate). Floors predicted before every run and hit exactly on BOTH interpreters each time: 3892/14/1 at `c5e84c9`, 3899/14/1 = 3914 collected at `d84a3c1`. Disjointness re-derived merge-base-form at each landing (main moved under the wave twice — c337's landing and the interface's ruling records — intersection empty every time). 15 consecutive ratified bypass events through this incarnation.

**THE GATE'S BLOCKING FIND WAS IN THE ONE SENTENCE THE LANE ADDED BEYOND ITS BRIEF, AND IT FAILED ON THE CONJUNCT THE GATE'S OWN BRIEF WASN'T AIMING AT.** s5's E2 bound (*"a record §4 makes append-only and no shipped verb removes an entry from"*) survived every removal-half measurement the gate brief prescribed — `supervisor/JOURNAL.md` unremovable (b), homes list unremovable (c), outcome store removable by `clean` exactly as the spec row says (d) — and failed on the SOURCING half: §4 of `multi-fleet.md` sources only the homes list, so read literally the bound admitted ONE record and un-grounded the two promotions its own commit lands. Spec-says-X/binding-says-Y, one level up in the criterion built to close it. *A brief that names the expected failure mode aims the gate; the gate that found B1 read the document's structure instead of running the brief's checklist — order both, trust the second.* Also: the gate brief's own parenthetical mechanism ("autoclean tombstone expiry" as the outcome-store remover) was WRONG — `cmd_autoclean` never removes an outcome record; the remover is `cmd_clean` → `_remove_worker_files`. The branch was right where the gate brief was wrong about its evidence.

**THE REPAIR WENT BACK TO ITS OWN LANE, AND THE LANE EXCEEDED THE LITERAL ORDER CORRECTLY.** B1 steered to the still-warm s5 fork rather than a fresh fix wave (fix-waves-mint-defects 7/7): one commit, wrong clause kept as quoted disclosure, both records re-sourced by section-anchor citation. The lane then fixed N2's identical false sentence in the SECOND pin file the order never named, citing the repo's own fixing-only-the-reported-site defect class in the commit — the right call, disclosed as one isolated hunk. **A steer to a lane that still holds its context beats a fresh fix wave at both cost and defect rate.**

**A GATE DISCLOSED ITS OWN CONTAMINATED RUN, WHICH IS WHAT MAKES ITS CLEAN NUMBERS TRUSTABLE.** w44-gs5 started a 3.10 floor run while its own mutants were still planted; the run failed exactly on M3's target, was discarded, re-run from a verified-clean tree, and the contamination recorded "so nobody finds that line in a log and reads it as evidence." *Never start a floor run while a mutant is on disk — and a gate that reports its own sequencing error is worth more than one that reports only green.*

**Ruling flow worked at speed: the ceiling raise (200k→400k/300k) arrived mid-wave, its build lane (`w44/ceiling`) was dispatched the same hour, and the E2/homes tension went from gate-measurement to its own Open docket line inside one wave.** c337 handed up two out-of-fence finds now on the successor queue: claim-nonce `:2572` (§8 build table carries the same dead-SessionStart-consumer claim, unnamed-file form that escaped the ruling's three-place search — mechanical under the ratified deleted-file rule) and a `tests/test_supervisor.py:2071-2076` docstring with the three-consumer enumeration. Plus gs5's N1: `RATIFIED_DISRUPTIVE`/`RATIFIED_ORDINARY` have NO spec-transcription pin (M5 moved `wait` between spec rows, whole suite green) — pre-existing, and the branch still narrowed the silencing surface by emptying UNCLASSIFIED.

## 2026-08-05 — operator ruling: ceilings raised to 400k/300k <a id="2026-08-05-ceilings-raised"></a>

**Ruled by Altai in-session through the interface, by direct directive.** Supervisor hard dispatch ceiling 200k → 400k; worker band top 200k → 300k; band entry keeps the 50k margin (350–400k / 250–300k, interface transcription — the hard numbers are the ruling if the margin misreads intent). Amends ratified three-tier §11.3/§11.4. Grounds: with boot costing 85–130k, the 200k ceiling produced ~45-minute handoff generations the same day the handoff protocol first went green live (two consecutive successful handoffs, wave 43) — succession churn outweighed the freshness benefit. Executes as a gated lane; until the code lands, the shipped 200k refusal governs. *The band was priced when boot cost ~40k; a threshold ratified against one cost structure needs re-pricing when the structure moves.*

## 2026-08-05 — operator ruling: the §5 criterion completed on four questions <a id="2026-08-05-s5-criterion-ruled"></a>

**Ruled by Altai in-session through the interface, minutes after the wave-43 supervisor filed it.** The MF §5 generative criterion (*worst irreversible effect in the wrong home*) ran out on four questions; all four ruled as the lane+gate convergently recommended. **E2: an irreversible APPEND is an irreversible effect** — a wrong-home append pollutes a foreign fleet's append-only record; six verbs classify accordingly. **B3: §5 gains the separating ground** *`wait` persists only transitions it observed; `release` alters ownership* — `wait`=ORDINARY; this repairs a measured defect in the ratified table itself (`wait`/`status`/`release` have byte-identical 9-site effect sets, so set-equality alone selects two classes at once). **E1: the dispatch is the act** — `sup-spawn`=DESTRUCTIVE; hand-maintained lint entry accepted. **E5 split out as its own open gate** (`doctor --repair`'s row justification indistinguishable from `peek`). Landing consequence ruled with it: both pin files extend (`RATIFIED_*` tuples + `DESTRUCTIVE_VERBS` lint at `tests/test_terminal_surface.py:848` — the miss that had the branch honestly RED), predicted 3651/14/1. Process note worth keeping: **the lane declined to route around its own failing pin and asked for the ruling — a RED disclosed with evidence beats a green obtained by weakening.**

## 2026-08-05 — operator ruling: the deleted-file-reference rule <a id="2026-08-05-deleted-file-reference-rule"></a>

**Ruled by Altai in-session through the interface.** `docs/specs/claim-nonce.md` names the deleted `bin/hooks/sessionstart_fleet.py` in three places; wave 42 measured them as three different kinds of object and the operator approved the split: **restate `:337` only** (a live normative consumer enumeration, false today — drop the dead consumer, keep the §6.3 key-set constraint, append a dated D7 note); **`:765` and `:1799` are NOT touched** (a receipt pinned `# at 091d5fa` that reproduces PASSED, and a quoted review argument — both true claims about a past tree). **Ratified general rule: a reference to a deleted file is only rot when it is a claim about the CURRENT tree; a pinned receipt and a quoted argument are claims about a PAST tree and are still true.** The counterfactual that motivated it: a blanket "fix all three" would have converted one real defect into two fabrications. Recorded in OPERATOR-GATES (Settled); the `:337` edit executes as a gated small edit under the running wave.

## 2026-08-05 — wave 42: four landings, and three measurements that were nulls wearing the shape of answers <a id="2026-08-05-w42-landing"></a>

**All four wave-41 branches landed on main; the inherited arithmetic hit exactly.** `w41/rc-constant` → `w41/pin-ruled-a` → `w41/mf-slice-a` → `w41/citations`, in that order, main `f457a57` → `e56d43a`. Predicted 3647 + 43 + 51 = **3741 passed / 14 skipped / 1 xfailed = 3756 collected**, and measured exactly that on **both** `py -3.13` and `py -3.10`, with each intermediate step predicted before running (3690/14/1 after rc, 3756 collected after pin, unchanged by the doc-only mf merge). Every merge's parents proved by `rev-parse HEAD^1 HEAD^2` rather than inferred from "the merge succeeded". Baseline re-measured at main before touching anything (3647/14/1) rather than inherited from the handoff — and it matched, which is evidence about the predecessor, not just about the number.

**THE DISJOINTNESS TEST, AS EVERY BRIEF STATES IT, IS THE WRONG TEST — AND IT FAILS TOWARD FALSE COLLISION.** `git diff --name-only <main> <branch>` is a two-endpoint diff: it reports the UNION of what main changed and what the branch changed. Run that way, all four wave-41 branches appeared to touch `docs/OPERATOR-GATES.md`, `knowledge/INDEX.md`, `knowledge/lessons.md` and `supervisor/JOURNAL.md` — files only *main* had touched — which would have serialized a wave that was in fact parallel. The correct form is against the merge-base: `git diff --name-only $(git merge-base main $b) $b`. The campaign brief's form was right only by coincidence of all four branches sharing base `28df49b`. **A disjointness test written as a diff between two tips measures two edit sets and reports them as one.**

**`changed in both` IS NOT A CONFLICT COUNT.** Three-arg `git merge-tree` reported 2 `changed in both` sections for `w41/citations` × main, inherited as "2 conflicting sections to resolve". The actual merge **auto-merged both** — `changed in both` means both sides edited the file, not that the hunks collide. Zero manual resolution was required. The predecessor's *count* was right and its *meaning* was wrong, and the wrong meaning is the one that sizes the work.

**AND THE MARKER GREP IS STILL A NULL AT THIS GIT — CAUGHT ONLY BECAUSE THE CONTROL RAN FIRST.** Wave 38's rule (*run any measurement whose good answer is 0 against a known non-zero input first*) was executed here as written: `grep -c '<<<<<<<'` over three-arg `merge-tree` output returned **0 for `w41/rc-constant` AND 0 for `w41/citations`** — the branch already known to collide. The control proved the grep blind before its zero could be believed for rc. **Two waves apart, the same instrument, the same null; the difference is that this time the control was mandatory.**

**MY OWN LOST-HUNK CHECK SHIPPED A DEFECT AND MY OWN CONTROL DID NOT CATCH IT.** The check reported 5 of 9 added lines missing from the merged tree — an apparent lost-hunk disaster. Cause: every flagged line begins with a markdown `- `, and `grep -Fqx "$line"` parses a leading dash as **a flag** (`grep: unknown option --`), so the check errored and the error read as "absent". I first diagnosed it as CRLF (`core.autocrlf=true`, a real hazard in this repo) and **that diagnosis was wrong**. The control I had written used a sentinel that did *not* start with a dash, so **it exercised every shape except the failing one** — 2026-07-27's *"a control that isn't controlled is worse than no experiment"*, reproduced by the person quoting it. Fixed with `grep -Fqx -e`, re-controlled with three sentinels (absent, dash-and-absent, dash-and-present): **9/9 present, both sides.** *A control must include the shape of the data it is controlling for, not merely an instance of the data.*

**THE PIN TIER DOES NOT RUN IN THE SUITE, AND ITS SKIP IS INVISIBLE FROM THE FLOOR.** `tests/integration/test_native_pin.py` is gated on `FLEET_LIVE=1` and skips all 6 tests by default — so the 6 sit inside the reported "14 skipped" and **a green full suite says nothing whatsoever about the native contract.** Stamping `state/pin-pass.json` off a full-suite run would certify a tier that never executed. Measured properly with `FLEET_LIVE=1`: **5 samples at 2.1.222 on the landed main, 6/6 every time.** Stamped 2.1.222 only after `w41/pin-ruled-a` was ON main — the ruled condition — and `fleet doctor` now reads `[PASS] pin-version: pin-test pass current (2.1.222)`. Reported as counts, never as a colour: the *tier* passes reliably; the *underlying record shape* remains the coin flip wave 41 measured (9 pooled samples, 5 published / 4 withheld). A read-only census over the live store's 164 result records reads **19 withheld / 145 published / 0 mixed** — spanning pre- AND post-gate eras, so it is NOT a current-rate measurement, but the **zero mixed** is an independent confirmation of ruling A's all-four-or-none invariant.

**THE TWO DEFERRED CITATIONS HAD OPPOSITE PROVENANCE, AND ONLY MEASUREMENT SEPARATED THEM.** `knowledge/lessons.md` → `verify_receipts.py:684` was **correct at `28df49b`** and was rotted **by this wave's own rc landing** (+205 lines to that file) → re-pinned `:704`; the lane's *"correct now, at risk on landing"* came true exactly as written. `docs/specs/fleet-index.md:538` → `stop_outcome.py:186-189` was **already rotted at `28df49b`** — no branch this wave touched `bin/hooks/` at all — → re-pinned `:255-258` and `:344-346`. The lane deferred the second one citing the `tools/verify_receipts.py` fence, **which never applied to that file**: right action, wrong argument, item already broken. *A deferral is a claim about a cause; the item can be broken for a reason the deferral never names.* Fixpoint proved **by construction** rather than by iteration: both repairs are same-length in-line substitutions (`1+/1-` per file, line counts identical), so no line moved and no new rot is creatable — one round, not the three the quiet-tree repair needed.

**A REFERENCE TO A DELETED FILE IS ONLY ROT WHEN IT IS A CLAIM ABOUT THE CURRENT TREE.** Wave 41 escalated *"`docs/specs/claim-nonce.md` cites the deleted `bin/hooks/sessionstart_fleet.py` in three places"* as three paragraphs needing re-decision. Measured, they are **three different kinds of object**: `:337` is a **live normative claim** (a consumer enumeration that is false today) and is the only one needing a ruling; `:765` is a **pasted receipt pinned `# at 091d5fa`**, the file *existed* at that commit, and the receipt reproduces (`68/69`, `EXIT: 0 (PASSED)`) — editing it would make a **verified receipt false**; `:1799` is **quoted review text inside a `>` blockquote**, which this repo corrects by append, never by rewrite. **A blanket "fix all three" would have converted one real defect into two fabrications.** Filed to the operator as a gate with that split and a recommended restatement; not ruled by the supervisor, because no meaning change lands on ratified text.

**DOCTRINE, ADDED THIS WAVE: a ROLLING document may never be line-cited.** `docs/NEXT-SESSION.md` is rewritten in place each session, so `NEXT-SESSION.md:133` names different text on different days while looking exactly like a durable citation — and unlike ordinary rot, **there is no correct line number to re-pin it to.** Cite rolling documents by section heading or by quoted phrase, or pin the claim to a commit. This is a statement about which SURFACES admit citation at all, and it generalises: a citation's validity depends on the target's rewrite discipline, not only on its current contents.

**PARALLELISM, EXERCISED AS THE OPERATOR ORDERED.** Two lanes (`w42-mf5` on `docs/specs/multi-fleet.md`, `w42-launch` on the README/getting-started surface) dispatched **before** any landing, on the merge-base-derived proof that neither file is touched by any wave-41 branch or by any main-side commit — the first wave to dispatch into an in-flight landing rather than after it. The a1 build lane stayed correctly blocked behind citations reaching fixpoint, because `w41/citations` edits `bin/fleet.py`, which is a1's whole file.
...
## 2026-09-09 — server ruling: keeper revives, supervisor is a swappable layer {#2026-09-09-keeper-revives}

Operator ruling, in-session through the interface (Telegram), superseding the 2026-09-08 "timer pages, human revives" ruling: **the keeper recreates the interface window and, on a dead supervisor, instructs the interface to relaunch it (keeper never dispatches; the interface runs `sup-spawn` on the `KEEPER:` line without waiting for the operator)**; **a supervisor at its context band gracefully notifies the interface and the two hand off together to a new generation**; **the interface is the operator's own persistent session, the supervisor is the swappable layer between it and the workers.** Recorded here by the interface tier; the OPERATOR-GATES Settled line is the operator's to write. Task file: `state/tasks/20260909-succession-ruling.md`.

**AMENDED THE SAME DAY BY THE OPERATOR, AND THE AMENDMENT IS THE VERSION THAT BINDS** — *"keeper must just instruct interface to relaunch supervisor."* Principle 3 as first recorded (above) said the keeper revives the supervisor; corrected: **the keeper NEVER runs `sup-spawn`.** It types a `KEEPER: … relaunch` line into `work:fleet` and the **interface** dispatches on that line without waiting for the operator. The keeper still recreates the interface window itself. **The two-live-body guard moved WITH the dispatch, onto the interface**: check `fleet sup-status --json` and the roster before any `sup-spawn`, and page the operator instead when the state is ambiguous — seized, claim `unknown`, a handoff in flight, or a releasing body still roster-live. *A document that lands principle 3 in its pre-amendment form is wrong; that includes the paragraph immediately above this one, which is left standing as the record of what the first pass recorded.*

**The graceful end of a generation is FOUR steps and it is ROUTINE, not an incident** — the framing carries as much as the steps, because the prose it replaced treated a supervisor ending as an escalation: (1) checkpoint with the successor queue; (2) **notify the interface** — one `SUPERVISOR:`-prefixed line typed into `work:fleet` with the keeper's own sanitising; (3) the handoff protocol, run with the interface; (4) `sup-release` only if the handoff is stillborn, after which the keeper pages and the interface relaunches.

**THE RULING WAS WRONG ABOUT ITS OWN MECHANISM AT STEP 3, AND THE CORRECTION IS THE MOST REUSABLE THING HERE** *(lane `w58-docs`, landed 2026-09-09; the ruling reached the lane through two documents and neither author opened the code)*. It says *"the interface … runs `sup-spawn` for the successor"*. **MEASURED: `cmd_sup_handoff_begin` dispatches the successor ITSELF** — it builds its own `[claude, "--bg", "-n", …]` argv and is the one `--bg` launch that deliberately does not go through `dispatch_bg`, handing that body a task file already carrying `sup-boot --handoff-inc --handoff-token`. An interface `sup-spawn` at that moment mints a **gen-0 body with no token** — a second live body over one `supervisor/GOALS.md`, the one condition the claim system exists to prevent. So the interface **watches** `sup-status --json` through a handoff and dispatches **only** on the stillborn path. *The general lesson: a ruling that names a verb has made a claim about code, and the lane landing it owes the grep — the behavioural half of this ruling was right and the mechanical half was a second live body.*

**`fleet sup-notify` IS A NAME, NOT A VERB, until a build lane ships it** — `grep -rn "sup.notify" bin/ tests/ docs/` returned nothing at `2a15dec`. The BEHAVIOUR binds (a sanitised one-line `SUPERVISOR:` message typed into `work:fleet`); every prose surface that names the spelling marks it unshipped, and none pastes a receipt, because a command nobody has run is a claim.
...
## 2026-09-10 — reap finished lanes at every wave boundary (8 GB host) {#2026-09-10-reap-at-boundary}

At 15:18:08Z the OOM killer hit the claude daemon scope on kz-work; the daemon shut down (cause=signal, live_workers=12) and every Claude session died with it, the supervisor body included. Of the 12, four were retired supervisor bodies and eight were idle finished workers, ~350 MB each; nothing had reaped them. The operator ruled: "yes, that should be the norm. previously i had been using my pc with 64gb ram so i didnt notice". AMENDED the same hour: reaping is a fleet MECHANISM, not a supervisor chore — `fleet sup-boot` and the stand-down verbs (`sup-handoff-complete`, `sup-release`) run autoclean and archive finished lanes and retired predecessor bodies themselves, and the boot bundle states the rule so every generation knows it (operator: "autoclean should just run when doing the supervisor stand down or supervisor boot process, not be done by the supervisor itself"; build item on mcx, `state/tasks/20260910-reap-at-boundary-AMENDMENT.md`); at most 3 live worker sessions on this host, Claude and Codex counted together; no dispatch under 1.5 GB available. Text: `state/tasks/20260910-reap-at-boundary.md` in the fleet home.
...
Operator ruling 2026-09-10T16:5xZ, verbatim: "dont spam astra, use with discretion and use the gpt5.6 models instead. we burned also 75% of the codex weekly limit too, well not fleet only but still". Supersedes the earlier "spam astra" line. Default mcx worker model is `gpt-5.6-luna` (omit `-m`), effort medium (high for `bin/fleet.py` build lanes); `gpt-6-astra` only with a written reason naming why 5.6 would fail, never for docs, tests, receipts or folds. With Claude at 77% and Codex at 75% of their weekly plans, waves are 1-2 lanes until 2026-09-15. AMENDED 21:0xZ the same day after the operator used a Codex usage reset: "dont go wild but you may use more heavier models at discretion" — astra at the supervisor's discretion where the model is the bottleneck, at most one astra lane in flight; Claude freeze unchanged. Text: `state/tasks/20260910-codex-budget.md` in the fleet home.

## 2026-09-10 — G-K8 ruled C: the keeper wakes through `fleet send`, never the daemon socket {#2026-09-10-gk8-c}

Operator, verbatim: "Gk-8 yes go for C". The keeper waker built in `851e797` opened the Claude daemon's private unix socket; that path is rejected and removed. The wake is `fleet sup-guard --do`: on `WAKE <body>` the keeper runs `fleet send` with a standing wake file; on `DISPATCH` it pages the interface (sup-spawn stays the interface's verb); on a limited body it pages once and waits for the horizon. Standing goal 3 ("zero writes to foreign surfaces") is unchanged and now holds by construction. Text: `state/tasks/20260910-ruling-gk8-c.md` in the fleet home. Four stalls on 2026-09-10 (13:54Z, 18:48Z, 20:06Z idle-after-dispatch; 15:28Z OOM) cost about five hours of idle fleet; this closes G-K6 wave 2.
