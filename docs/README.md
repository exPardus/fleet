...
| [`launch-readiness.md`](launch-readiness.md) | USERS | **What stops an external user today**, measured rather than assumed, ordered by what blocks a first successful use — plus an explicit list of what was *not* verified. Read it before you install if anything below surprises you. |
...
| [`OVERNIGHT-2026-07-23.md`](OVERNIGHT-2026-07-23.md) | INTERNAL | Run log of the first unattended overnight campaign (operator asleep), opening with a morning TL;DR of what shipped and what awaited a push. |
| [`AUTONOMOUS-2026-07-24.md`](AUTONOMOUS-2026-07-24.md) | INTERNAL | Day-3 autonomous run log — and the record of the four-councilor + synthesis mechanism used to route operator-gated items while the operator was away. |
| [`AUTONOMOUS-2026-07-26.md`](AUTONOMOUS-2026-07-26.md) | INTERNAL | Day-4 autonomous run log: spec-backlog churn toward launch readiness. |
| [`mf-oracle-m1-evidence.md`](mf-oracle-m1-evidence.md) | INTERNAL | Read-duplication measurements harvested from live session transcripts — the economics evidence behind fleet-index M1. |
| [`decisions/`](decisions/) | INTERNAL | Operator-owned rulings and the council syntheses taken provisionally in the operator's absence, pending ratification (currently the §7 exemption-envelope pair). Distinct from `OPERATOR-GATES.md`, which is the open queue; this is the record of how specific ones were answered. |
| [`specs/`](specs/) | CONTRIBUTORS | One spec per phase/topic — all 15: `native-substrate.md`, `autoclean.md`, `claim-nonce.md`, `fleet-index.md`, `three-tier-command.md`, `multi-fleet.md`, `graceful-succession.md`, `portability.md`, `providers.md`, `terminal-surface.md`, `phase-2-watchtower.md`, `phase-3-telegram.md`, `phase-4-webui.md`, `phase-5-intelligence.md`, `phase1-hardening-kernels.md`. Each carries a `Status:` line — and the vocabulary is wider than any fixed list (observed: `stub`, `DRAFT`, `ready-for-build`, `ready-for-gate`, `spec-of-record`, `PARKED`, `SUPERSEDED`, `BUILT AND SHIPPED`), so **read the spec's own status line rather than trusting an enumeration here.** **`ready-for-build` means reviewed and buildable, never shipped.** Two worth knowing before you read the rest: [`multi-fleet.md`](specs/multi-fleet.md) is ratified ready-for-build and **unbuilt** — until it lands there is one fleet home per machine — and [`portability.md`](specs/portability.md) is self-declared **SUPERSEDED**; do not build against it. |
| §18 of [`SPEC.md`](SPEC.md) | CONTRIBUTORS | The shipped-milestone list — **but it is stale by two milestones.** M-0/M-A/M-B/M-C are recorded there; **M-D and M-E shipped afterwards and are not folded in.** For anything past M-C read [`PLAN-PROGRESS.md`](PLAN-PROGRESS.md) and [`NEXT-SESSION.md`](NEXT-SESSION.md) instead. |
| [`proposals/`](proposals/) | INTERNAL | Standing proposals awaiting an operator decision — currently the GOALS sync/tier-chain proposals, the sup-spawn choreography record, and [`github-settings.md`](proposals/github-settings.md) (exact `gh repo edit` commands for repo discoverability; nothing applied until the operator runs them). |
...
| [`reviews/ULTRAREVIEW-2026-07-30.md`](reviews/ULTRAREVIEW-2026-07-30.md) | INTERNAL — **OPEN priority queue** | The whole-repo adversarial sweep of 2026-07-30: 133 agents, 14 dimensions, every finding put to two independent skeptics that were told to refute it. **27 CONFIRMED / 23 REFUTED / 19 unvetted**, priority-ordered P0→P3 with the refutations kept as a record. Unlike the reviews above, this one is *not* history — it is the open fix list, and every defect in it was invisible to a green 2825-test suite. |
