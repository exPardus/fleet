...
| Doc | Audience | What it's for |
|---|---|---|
| [`SPEC.md`](SPEC.md) | CONTRIBUTORS | The canonical architecture: registry schema, the nine numbered load-bearing invariants, every CLI command's contract, the Windows launch detail. Read this before changing `bin/fleet.py`. |
...
| [`specs/`](specs/) | CONTRIBUTORS | One spec per phase/topic (`native-substrate.md`, `autoclean.md`, `claim-nonce.md`, `three-tier-command.md`, `portability.md`, `providers.md`, `terminal-surface.md`, `phase-2-watchtower.md`, `phase-3-telegram.md`, `phase-4-webui.md`, `phase-5-intelligence.md`, `phase1-hardening-kernels.md`). Each carries a `Status:` line (`drafting` / `ready-for-build` / superseded) — check it before assuming a spec describes shipped behavior. |
...
| [`superpowers/`](superpowers/) | INTERNAL | Working specs and execution plans for whatever's actively being built — right now, the native-substrate pivot (`specs/2026-07-13-native-agents-pivot-design.md`) and its build plans. More volatile than `docs/specs/`; this is where design happens before it's stable enough to move there. |
