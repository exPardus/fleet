...
## AMENDMENT 2026-09-09 — the interface revives, and a generation change is routine

**Operator ruling 2026-09-09 with its own same-day amendment** (`state/tasks/20260909-succession-ruling.md`; `knowledge/lessons.md#2026-09-09-keeper-revives`). It SUPERSEDES ruling 1's revival clause and the "human message IS the trigger" narrowing above. **Superseding is not deleting:** the 2026-09-08 text stays where it is, dated, as the record of what this design was built against. Wherever this document below still reads *"dispatch only when the operator says so"*, *"revival is a human message"*, or *"auto-revival ruled out"* as a statement about who acts, read it through this section.

1. **The interface is the operator's own session.** Persistent. Never recycled by fleet or the keeper for context reasons; only the operator's `recycle interface` retires it.
2. **The supervisor is a swappable layer** between the interface and the workers. Reaching its context band is **routine, not an incident**: the generation ends gracefully and the next one takes over with no operator action.
3. **The keeper still never dispatches — and it no longer waits for a human either.** On a dead-or-released supervisor with GOALS active, the keeper types a `KEEPER: … relaunch` line into `work:fleet`, and **the interface runs `sup-spawn` on that line without waiting for the operator.** The keeper keeps recreating the interface window itself. **The two-live-body guard moves to the interface**: check `fleet sup-status` / `--json` and the roster before any `sup-spawn`, and page the operator instead of dispatching when the state is ambiguous — seized, claim `unknown`, a handoff in flight, or a releasing body still roster-live.

**Graceful end of a supervisor generation — four steps:** (1) checkpoint with the successor queue; (2) **notify the interface** with a one-line `SUPERVISOR:`-prefixed message typed into `work:fleet` using the keeper's own sanitising *(verb `fleet sup-notify`; **NAME UNSHIPPED at `2a15dec`** — `grep -rn "sup.notify" bin/ tests/ docs/` returns nothing; a sibling build lane owns the shared helper plus the verb, so the behaviour binds and the spelling is provisional)*; (3) the handoff protocol; (4) only if the handoff is stillborn, `sup-release` — after which the keeper pages and the interface relaunches.

**WHERE THE RULING IS WRONG ABOUT THIS DESIGN'S OWN MECHANISM, stated rather than smoothed over.** The ruling's step 3 reads *"the interface (or the keeper, if the interface is absent) runs `sup-spawn` for the successor"*. **MEASURED at `2a15dec`: it does not, and must not.** `cmd_sup_handoff_begin` in `bin/fleet.py` builds its own `[claude, "--bg", "-n", <name>, …]` argv and launches the successor **itself** — it is the one `--bg` launch that deliberately does not go through `dispatch_bg` — handing that body a task file that already carries `sup-boot --handoff-inc <id> --handoff-token <tok>`. A `sup-spawn` from the interface during a handoff would mint a **gen-0 body with no token**, i.e. a second live body over one `supervisor/GOALS.md`, which is the exact condition this whole design exists to prevent. The interface's role in step 3 is therefore to **watch** `sup-status --json` until the claim moves, and to dispatch **only** on the stillborn path (step 4). The ruling's *behavioural* requirement — that the two tiers work the succession together, with the interface informed and able to act — is landed in full; only its mechanical clause is corrected. Filed for the operator in `docs/lanes/w58-docs.md`.

**Sections of this document the amendment reaches:** §3.1 (the interface's launch prompt, "step 5 is replaced by…"), §3.3's `supervisor-dead` page text (`await operator before sup-spawn` — the keeper's shipped string, a sibling lane's to change), §5's first failure-mode row ("waits"), and §8's auto-revival exclusion — which is **still an exclusion**: the timer never dispatches under this amendment either.
...
| unpushed | commits reachable from `HEAD` that are on no remote ref, > 0 for > 6 h (age from the oldest such commit; when no remote-tracking ref exists at all, or the count cannot be read, the keeper reports on stdout and pages nothing). **Amended 2026-09-09 (lane `w56-keeperref`): this row specified `git rev-list --count origin/main..main`, a fixed ref pair.** On kz-work, where work happens on `server/persistent-fleet`, that saw **2** while **21** commits sat on one disk, then kept reporting **2** after a push left **0** at risk — wrong in both directions from one cause. The naive replacement is a trap the lane measured and avoided: `HEAD --not --remotes` counts an entire history, exit 0, in a repo with no remote-tracking refs | `KEEPER: <n> commits unpushed for <h>h. Push or explain.` |
...
|---|---|---|
| supervisor releases at ceiling, nobody home | keeper `supervisor-dead` within 15 min; interface reports in topic and — **since the 2026-09-09 amendment** — runs the two-live-body guard and relaunches, instead of waiting | ccgram |
