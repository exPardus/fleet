...
| G11 | Usage-limit wall under `--bg`: what does the roster show (state literal?) and what reaches the Stop-hook payload / transcript tail when a plan limit parks a session? **Cannot be induced on demand** — DISPUTED-by-nature. | no | design must feature-detect (§5.1.1); the contract entry is written on the first NATURAL occurrence and pin-tested thereafter |
