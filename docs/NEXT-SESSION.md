...
1. **Claim-nonce spec slice** (adjudication items 1–3, and it independently fixes the handoff-fork gap + the two claim warts found live this campaign: `sup-boot` refuses its own holder on a stale heartbeat, and can't distinguish an operator-authorized `claude stop` from a daemon restart → no `sup-release` verb). Spec task → dual-lens review → build **gated on operator ratification**. This unblocks a re-drafted three-tier.
