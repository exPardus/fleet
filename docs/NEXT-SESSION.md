...
6. **Decide what the external-provider work is now.** `origin/feat/worker-providers` was reconciled into this tree and brought two documents with it. `docs/superpowers/specs/2026-07-21-worker-providers-native-design.md` still carries `**Status:** approved design — ready for implementation plan` while its own §4.2 records the gating spike as **NEGATIVE** ("Do not implement §4 as written") — the header and the body disagree, and only the operator can settle which one stands. `docs/longcat-fleet-usage.md` is the working alternative: a per-`CLAUDE_CONFIG_DIR` isolated daemon namespace, verified end-to-end 2026-07-21. Ask: re-status the design doc, and does `docs/specs/providers.md` get re-based on the negative result or parked?
