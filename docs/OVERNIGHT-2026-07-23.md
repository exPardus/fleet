...
9. *(2026-07-24 addendum)* **"No push, no merge" is ambiguous to a worker with a remote**: `residual-fixes` pushed its feature branch and opened draft PR #8, reading the rule as "no push to main". Work was sound; remote branch deleted post-merge. Fence phrasing fix for task templates: "no `git push` of ANY ref". Same class as the heredoc finding: fences must be stated in the worker's operational vocabulary, not the manager's intent vocabulary.
