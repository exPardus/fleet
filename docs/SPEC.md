...
**PID liveness — three-way probe (v2.1, M4) — SHIPPED (verified 2026-07-09).** `probe_liveness` (`bin/fleet.py`) returns three verdicts, never a bare alive/dead boolean; the `ACCESS_DENIED` marker + `Get-CimInstance` fallback, the never-demote-alive-unknown rule, the retry-once-before-`dead` transition, and `_doctor_check_unreadable_starttime` all exist in code. The one part of this paragraph still `[UNBUILT — owned by C2 hardening kernel item 9]` is the §5 doctor **elevation-mismatch** check. The stored `turn_pid`/`turn_pid_ctime` classify into exactly one of three verdicts:
