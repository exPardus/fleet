# Proposal: `supervisor/GOALS.md` replacement — three-tier doc-sync

**Status:** proposal only — `supervisor/GOALS.md` is operator-owned; this file is the doc-sync
deliverable in its place (int-docs wave, 2026-07-23). Nothing here is live until the operator
commits it into `supervisor/GOALS.md`.

**Authority:** `docs/specs/three-tier-command.md`, RATIFIED 2026-07-23 (OPERATOR-GATES settled
entry; `knowledge/lessons.md#2026-07-23-three-tier-ratified`). Its §12 names GOALS.md as the
operator-owned policy surface owing these changes.

## What changes, and why (each cites its spec section)

1. **New "Tier policy" section — the role→tier table (§3.1) and the supervisor preference chain
   `[top, second]` (§3.5).** GOALS.md is the policy surface the chain is read from — never a code
   constant. Today's Anthropic resolution (Fable 5 / Opus 4.8 / Sonnet) is illustrative, never
   normative.
2. **Worker-model allowlist: Opus or Sonnet, never Haiku (§3.4).** Doctrine in GOALS until the
   spawn-time guard is built (`[UNBUILT — three-tier build slice]`). Haiku remains legal as a
   subagent *inside* a worker session. On a single-model provider namespace the allowlist is moot
   (§3.3) — it is Anthropic-namespace policy, not a universal invariant.
3. **Cost-frugality section rewritten.** The "cheapest model that can do the job" bullet is
   superseded by the ratified tier table (task directive + §3.1): the supervisor is deliberately
   TOP tier — planning quality is the scarce resource — and worker choice is second-vs-third tier
   per task, never Haiku. The remaining frugality bullets (beat sizing, don't re-derive, batch,
   spend-where-it-compounds, watch the horizon) survive unchanged; they were never about model
   choice.
4. **Context band 150–200k, supervisor AND workers (§11, §11.4).** Replaces the per-incarnation
   budget-cap constraint line: the third-docket cap doctrine (OPERATOR-GATES, M-F budget envelope,
   2026-07-23) retired fleet-enforced spend ceilings; the band is a freshness mechanism, not a
   budget (§11.5).
5. **Bypass-permission acknowledgement (§10.2).** The supervisor runs under bypass permission mode
   in the fleet repo (earned-privilege doctrine) — §10.2 requires this stated explicitly in GOALS
   before it ever runs unattended.
6. **Checkpoint-cadence doctrine (§3.5.3(b), §12 wave-4 row).** The supervisor checkpoints its plan
   to the journal at each wave boundary, because that cadence is exactly what bounds the plan loss
   when a usage-limit fallback fires — the only mitigation available for that loss, so it belongs
   in policy rather than in a reviewer's memory.

Everything not listed above is carried over verbatim from the current GOALS.md.
...
ratified three-tier spec (2026-07-23, pending operator adoption). Becomes
live when the first supervisor incarnation boots (spec §4 boot ritual).
Until then the acting manager session treats this file as its campaign
north star.
```
