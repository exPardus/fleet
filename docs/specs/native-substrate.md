...
> **[OBSERVED 2.1.212 — RATIFIED 2026-07-23 by Altai; three dead-daemon manager-report claims WITHHELD — see inline `RATIFICATION WITHHELD` markers]** The CLI moved 2.1.207 →
...
- **[OBSERVED 2.1.212 — RATIFIED 2026-07-23, EXCEPT the dead-daemon rm/stop sentence (WITHHELD, inline)] The daemon is TRANSIENT — every CLI verb that needs it can fail when nothing holds it open.** (receipts: `docs/reviews/CLAUDE-2.1.212-CONTRACT-2026-07-17.md` §Q2, §Q4) `claude daemon --help` @2.1.212: *"Service install is disabled in this version — the daemon runs on demand and exits when the last client disconnects."* It starts on demand at dispatch (`origin=transient` on every `daemon start` line in the log) and idle-exits ~5s after the last client disconnects **and** its live-worker count reaches 0. A live `--bg` session holds it open; so does an open `claude agents`. **[RATIFICATION WITHHELD 2026-07-23 — manager-report-only, unverified by two waves; capture on a quiet machine first:]** With the daemon down, `claude rm`/`claude stop` fail (rc=1, *"the background service may be restarting"*) and **do not revive it — only a dispatch does**. `claude agents --json [--all]` still answers, from the on-disk roster/jobs snapshot.<br>**Why this bites fleet specifically:** the hygiene tier is, by construction, the code path most likely to meet a dead daemon and the least able to revive it — `fleet archive` and the `fleet autoclean` scheduled task only ever run against workers that are *not* live, which is exactly the zero-live-worker state in which the daemon has idle-exited. A hygiene pass must never dispatch to wake the daemon (that mints a new billable session); it defers loudly and retries next pass. **Corollary for every observer:** a fleet process running *inside* a `--bg` session (a supervisor, a worker, this contract's own probes) always sees a live daemon and therefore **cannot reproduce the dead-daemon path at all** — identical commands genuinely behave differently from an interactive session on a quiet machine. Anything measuring rm/stop must record `claude daemon status` alongside its verdict, or it is measuring an uncontrolled variable (this is precisely what made the pin tier's `test_5_pin_archive_rm` look RED — §Q1.4).
- **[OBSERVED 2.1.212 — RATIFIED 2026-07-23] `claude daemon stop` has machine-wide blast radius — never automate it.** Its own `--help` reads *"Shut down the supervisor and terminate background sessions"*, with `--any` (also stop a transient daemon) and `--keep-workers` (leave detached sessions running). Default behavior kills background sessions **that are not the caller's**, including other operators' and other fleet workers'. No fleet code path, test, or probe may run `daemon stop`/`daemon uninstall`; a probe that seems to need it is BLOCKED, not run (§Q2).
- **[OBSERVED 2.1.216 — RATIFIED 2026-07-23] A STALE `~/.claude/daemon.lock` + PID REUSE is a machine-wide dispatch outage that every fleet surface reported as healthy.** (receipts: manager's own captures, interactive session, quiet machine, zero live workers, 2026-07-21; `docs/PLAN-PROGRESS.md` M-E row, 9th live catch) For ~16h **no** `--bg` dispatch on the machine could succeed — no spawn, no respawn, no steer, no resume — and **`fleet doctor` reported 21 PASS / 0 FAIL.**<br>**Symptom.** `fleet spawn` died with `fleet: pin-w1: native spawn failed -- --bg dispatch exited 1: Starting background service` / `Couldn't reach the background service (background service did not become reachable within 45s) - run 'claude daemon status'`. `claude daemon status` answered `not running`, `control.sock: unreachable (connect ENOENT \\.\pipe\cc-daemon-*-control)`, `bg workers: 1 in roster.json (control unreachable)`, `roster.json: updated 45962s ago`.<br>**Root cause.** The daemon that took the lock at `2026-07-20T23:25:27.750Z` (`pid=15740`, `origin=transient`) died. Windows recycled PID 15740 onto **`WacomHost`**, a service process whose `StartTime` is **unreadable to a normal-token probe** (`(Get-Process -Id 15740).StartTime` → null, access denied). Every later start therefore saw a live PID 15740, concluded another daemon held the lock, and exited — `daemon.log`: `[2026-07-21T15:07:01.348Z] [supervisor] ─── daemon start ─── version=2.1.216 pid=34724 origin=transient` then `[2026-07-21T15:07:01.868Z] [supervisor] another daemon won the lock race (pid=15740) — exiting`. The lock file **does** carry a discriminator — `procStart: "639202047274516770"` (.NET ticks, LOCAL = `2026-07-21T04:25:27` = `23:25:27Z` at UTC+5, i.e. the DEAD daemon's start) — and it was not decisive.<br>**`claude daemon stop --any` is NOT the remedy.** It printed `no daemon running` and **left the lock in place**. Removing `~/.claude/daemon.lock` restored dispatch immediately.<br>**The native contract itself was NOT broken.** After lock removal the pin tier went **6/6 green on 2.1.216**. This is a *substrate/lifecycle* failure, in the same family as the transient-daemon row above — do not read it as contract drift.<br>**Fleet's adaptation (M-E, shipped on `me/daemon`).** (1) `_doctor_check_daemon_wedge` FAILs on the wedge signature — **one or more** `another daemon won the lock race (pid=N) — exiting` lines naming the lock's own pid, dated at least 300 s AFTER the lock's `startedAt`. The **lock-age gate is the whole discriminator**; a count/span requirement is not, and the first shipped version (`89d1928`, ≥3 refusals spanning ≥300 s) **could not fire on this very incident** — refusal lines are demand-driven, one per attempted dispatch, and the outage's entire evidence window (first failed dispatch `15:07:01.868Z` → operator remedy `15:09:42.909Z`) was **161 s and exactly one refusal**. Fixed on `me/daemon` after the hostile review `docs/reviews/ME-DAEMON-REVIEW-2026-07-21.md` §M1 (that file lands with branch `me/daemon-review`; merge both); the age comparison is SIGNED, so refusals predating the current lock never indict it. It reads two files, shells out to **nothing** (a probe that starts a daemon is worse than no probe: the vendor's own warning is that `claude agents` restarts it), and every unknown is a PASS-note — a false FAIL in a command read every session is worse than a late catch. (2) `dispatch_bg` classifies the unreachable-service stderr and names the stale lock, `fleet doctor` and the remedy instead of echoing the vendor string. Neither path writes anything under `~/.claude`; fleet never removes the lock for you.<br>**Deliberately NOT used as a signal:** the lock's `procStart` / any PID-liveness probe. On the one case this exists for, the recycled PID's start time was **unreadable**, i.e. INDETERMINATE — and an unreadable start time must never be read as "still ours". `probe_liveness` / `get_process_info` / `boot_identity` no longer exist in `bin/fleet.py` (deleted in the section-6 pivot; grep receipt @`ac3e34d`: zero matches), so this would also have meant a new PLATFORM adapter method with no POSIX backend.<br>**Provenance / probe context.** The outage, the `daemon status` text, the `daemon.log` lines, the lock JSON, the failed `stop --any` and the post-removal 6/6 are all **manager captures from an interactive session**. The M-E worker that built the fix is itself a `--bg` session and therefore *cannot* reach the wedged-daemon state (same clause as the transient-daemon row): its FAIL/PASS directions are pinned by fabricated fixtures in `tests/test_native_daemon_wedge.py`, and a live end-to-end reproduction is **[MANAGER-VERIFICATION REQUIRED]**. **[UNVERIFIED — no POSIX box]** the POSIX `daemon.lock`/`daemon.log` filenames and the POSIX log line format were not observed; if either differs the check finds no refusals and returns a PASS-note, which is the intended direction.<br>**Ratified 2026-07-23.** Still wanted: capture the wedge once more if possible, and when you do, capture `~/.claude/daemon.status.json` and `~/.claude/daemon/roster.json` **at the same moment as `daemon.lock`** — they are the only candidates that could close the *proactive* gap (a wedge on a machine where nobody has dispatched yet emits no lock-race refusal at all, so no log-derived check can speak), and neither can be specified from the artifacts now in hand. `daemon/roster.json` mtime is NOT a candidate as a liveness signal and was refuted on this machine's own data: it is event-driven, and during this very incident it was already 1.59 h stale while the daemon was provably healthy (roster written ~`02:20:59Z`; daemon proven alive by `[2026-07-21T03:56:41.452Z] [supervisor] auth: proactive refresh succeeded`). The lock-age gate is calibrated against all four real lock-race refusals in the week's log — three benign at 0.647 s / 0.532 s / 0.450 s, the wedge at 56 493.5 s, a 4.9-order-of-magnitude separation — so the *threshold* is not the open question; the proactive gap is.
...
**Status: `[OBSERVED 2.1.212 — RATIFIED 2026-07-23 by Altai]`, except the three
dead-daemon manager-report claims, which carry inline `RATIFICATION WITHHELD`
markers pending a quiet-machine capture (G9 probe). Nothing here was
