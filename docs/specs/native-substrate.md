...
- **Roster reads: zero changes**, on evidence. An idle-exit is gated on
  `live_workers=0` (16/16 receipts) and so cannot strand a live-looking entry;
...
3. **The `cause=upgrade` shutdown path** (3/3 shut down with live workers, up to
