...
- **The nameplate carries the HOME's identity on a multi-fleet machine** *(added 2026-09-10, operator ruling G-K5 Reading A: "fleet statusbar must be different per fleet home")*. `[fleet]` becomes `[fleet:9c3a]`, where the tag is `fleet.home_tag()` — `HOME_TAG_HEX` (4) hex digits of a digest over `fleet.home_identity()`, the same key the homes-list fold uses. Per-home *resolution* was already built (slice (d), §5); what was missing is that the row said **which** fleet it had resolved, so two windows showed two different counts with nothing naming either. The tag is **inside** the nameplate, sharing its hue: colour is rationed on this line (grey is reserved for `dead`, every other hue is a documented status), so a per-home hue would have to be drawn from the same palette the statuses use.
  - **A digest, not an abbreviated name.** Bounded width is non-injective over paths for *every* scheme, so the question is what each character buys. A fleet home is typically a clone or worktree of this repo, so `/home/a/proga/fleet`, `/srv/fleet` and `/mnt/w/fleet` all abbreviate to `fleet` — a name-shaped tag would render identical bytes for three different homes, which is P1-13 on the one field added to stop that confusion. A digest also **launders** the path: a member path comes out of `~/.claude/fleet-homes.list`, which since slice (d) the reader does not necessarily control, and hex digits cannot carry an escape, a CR or a bracket into the one token `_safe` strips brackets to protect.
  - **A single-home machine renders no tag at all** and is byte-identical to the pre-ruling row. The gate is `resolve_blob_home`'s existing verdict: `HOME_SINGLE` — which is also its error value, so a resolution that *dies* also degrades to the shipped row. §5's arming paragraph already bound slice (d) to "with a determinate population of <2: byte-identical to today"; the tag inherits that rule rather than inventing a second one.
  - **Tagged on exactly the two states that render a roster** (`lookup`, `default`). The §5 step-5 terminus and the ambiguity word are untagged **by design**: both mean *there is no home to name*, and §5's refusal contract is "facts + the `fleet homes` view, never a chosen home". A corrupt *default* home therefore renders `[fleet]: no home -- registry unreadable` and not a tagged fault row — it fails `home_is_initialized` and falls to step 5. **Residual, recorded rather than fixed:** on a multi-fleet machine that line does not say which home was unreadable. Naming it is a change to §5's refusal contract and belongs to a gate, not to a renderer.
  - **No new read.** `home_tag` is a pure string function over a path the process already holds; the homes list was read once by `resolve_blob_home` before this field existed. D4 is untouched: no lock, no probe, no write, no quarantine.
  - **The legend.** Four hex digits name no directory, so the mapping is readable on two verb-tier surfaces that use the same pure function: `fleet homes` gains a tag column (and **names a tag collision** rather than letting two homes quietly share a nameplate), and `fleet home --tag` prints the resolved home's tag. Bare `fleet home` is unchanged — `$(fleet home)` is substituted into paths by the skill, the slash commands and the briefs.
...
| a record field whose TYPE is not the schema's | the field renders `?type`; **the row still prints** | unaffected — the table re-derives its status column |
...
**The `?type` row, and why the last row of that table is not an answer** *(added 2026-08-09, multi-fleet slice (d), discharging gate `w50-gd2`'s MAJOR)*. Once slice (d) let the statusline read a home the operator does not control, a registry record's field TYPES became foreign input alongside its text. `_registry_corrupt_reason` constrains the registry to an object of objects and says nothing about a record's fields, so `{"status": []}` is a *valid* registry — and an unhashable bucket key raised inside the render, fell into *"any unexpected exception → print nothing, exit 0"*, and **removed fleet's own row from the operator's screen at rc 0 on every refresh with no diagnosis**. Driven four-cell: at the merge base the same record was harmless, because no home but `$FLEET_HOME` could affect the render at all.

Two rules follow, and they are the shape of the fix rather than its patch:

- **The last row is a backstop, never a degrade path.** A shape that is foreseeable at a trust boundary must be answered by a word, not by the swallow — the swallow is what makes a failure silent, so catching *more* there is the same defect with more surface.
- **The narrowing precedes the sanitiser.** `fleet_statusline._safe` is a TEXT sanitiser: printable ASCII, bounded, brackets neutralised. Its guarantees are about characters and it has none about structure, so a wrongly-typed field is narrowed to a word fleet authors (`fleet.registry_status` → `?type`) *before* any sanitiser sees it, and `_safe` refuses a non-string rather than stringifying one. Coercing with `str()` would also make the render total and is refused: `_safe(["idle"])` and `_safe("['idle']")` are both `('idle')`, so a malformed record and a record forging that repr would render identical bytes on the one surface the operator reads continuously — P1-13, one field over. `?type` is distinct from `?` (absent or null) for the same reason, and is exempt from the unknown-bucket cap so that three hostile statuses sorting earlier cannot suppress it.
