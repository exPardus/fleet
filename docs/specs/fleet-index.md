...
- Literal tabs in `sig` and in `name` are escaped as `\t`; newlines stripped. Escaping is applied at PARSE time, not at render time, so a row from a fresh parse is byte-identical to the same row read back off a shard.
- Sorted by `(line, end, name)`, not by name: a source edit perturbs a contiguous region rather than scattering hunks. That key is the shard's byte order **and** the order every read path hands rows back in — one order, or a caller sees a different answer depending on whether the shard had just been refreshed.
- **16 hex, not 8** (fix wave 2, 2026-07-27). 8 hex is 32 bits: a measured search over 109,096 two-line modules, under a second of CPU, produced two valid sources sharing a `sha256[:8]` *and* a line count — the whole staleness key. Swapping them served the wrong file's symbols with `refreshed: False` while `index status` reported nothing stale. A shard carrying the old 8-hex header is simply unreadable to this reader, so it takes the stale path and is repaired on first read; there is no migration step.
- A line ends at `\r\n`, `\r` or `\n` — `bytes.splitlines()`'s rule, and the header's line count and the parsers' line numbers both use it. Deliberately not `str.splitlines()`, which also breaks on `\x0b`, `\x0c` and `\u2028`, and deliberately not `\n` alone, which counted a CR-only file as one line while `ast` numbered its symbols 1..N — a shard whose own rows cited a line past its stated end.
...
**"Module-level" means `tree.body`, and that restriction is stated rather than left to the reader** (fix wave 2; gate-idx-rs MINOR). Only the module's top-level statement list is walked, so a definition guarded by `if TYPE_CHECKING:`, `try: import X except ImportError:`, `with`, or a module-level `for` is not indexed, and a tuple-unpacking assignment (`A, B = 1, 2`) yields no `const` rows because its target is an `ast.Tuple`, not an `ast.Name`. Both are ordinary Python shapes. This is under-inclusion, which is the safe direction — a symbol the index does not know about degrades to Read/Grep (§9), whereas an over-inclusive walk would put unreachable names in the lookup namespace. Widening it is additive and needs no format change.

**Enumeration is fenced in both directions.** The build walk does not descend a nested repository boundary (a directory holding a `.git` entry, file or directory — §11.1's predicate, applied downward instead of upward) and does not follow a reparse point (a symlink, or a win32 junction, which `os.walk` otherwise descends happily). Without the first, a parent `fleet index init` wrote shards for `.claude/worktrees/w1/*.py` — files on another branch, in a tree with an index of its own; without the second, a `mklink /J` inside the root pulled a file from outside it into the index under a shard path claiming it lived there. The root itself is exempt from the boundary test: it is nearly always a checkout.
...
`_index_posix_rel` is **shared**, and that has a second consequence worth stating because it was missed the first time it was measured: `fleet index update --files ../x.py` normalises through it too, so a guard that raises there pre-empts `_index_files_arg`'s own `startswith("../")` refusal *with a different message*. Verified by injecting the guard: the compose path was the reported casualty, but `fleet index update`'s refusal wording changed as well.

That consequence is now **settled the other way, and deliberately**. The first answer was to re-word the canonicaliser's refusal back into `_index_files_arg`'s own phrasing and keep this site's `rel == ".."` check as well. Both halves are dropped: the check, because the table row above states containment as *one* guard asked of every caller and a second one here is exactly what let the two spellings drift apart — this site rejected a leading `../` while the canonicaliser waved an interior one through to the prune path; and the re-wording, because it existed only to make the second check's message survive. `_index_posix_rel` raises `IndexPathError`, which is a `FleetCliError` and whose message already carries "outside the index root", so the verb's refusal still names the root — it is simply the canonicaliser's sentence rather than this site's. What a caller may rely on is the refusal and the root it names, not which layer wrote the string.
| Some files indexed, some failed | `indexed N … failed M` on stdout, per-file warnings on stderr, and a **distinct exit code (6)** — not the 1 every refusal already carries. The two states need different next actions: after a refusal nothing was indexed and the operator runs `init`; after a partial failure the index exists, most of it is current, and the operator retries the failures. Collapsing both onto 1 threw that away, and exit 0 read as total success to any caller that checks a return code instead of parsing the count line. |
| A path handed to the index that is not inside it | `IndexPathError` (a `FleetCliError`, so exit 1) and **nothing is written, read or unlinked**. Two guards, because containment is two questions. **Spelling:** a rel is refused for a `..` segment anywhere and for not being *relative* — a drive-qualified `C:/victim/x.py` carries no `..`, and `root / rel` is a join that pathlib resolves by **replacing** on an absolute right-hand side. The test is `drive or root`, never `is_absolute()`: `C:x.py` is drive-*relative*, so `is_absolute()` is False, and it still replaces a root on another drive. Asked of win32 syntax on **every** platform, because a rel is persisted and an index built on POSIX can be read on win32. **Filesystem:** the source and the shard must each *resolve* inside their tree, which is the half no string analysis can reach — `link/secret.py` is an ordinary rel until `link` is a symlink or a junction. Both sides are resolved, never just the candidate, or a root reached through a link stops containing its own files. |
...
**Platform-invariant matching semantics (win32 pinned).** Exact and dotted-tail matching are byte-wise **case-sensitive on every platform**. Shard-relative source paths use **forward slashes on every platform** — in the shard tree layout (§5), in `--path` matching, and in `<path>:<line>-<end>` output (§11.4) — so §12's golden tests are byte-identical across OSes.

**The glob dialect is `_index_glob_match`, one matcher for the whole family** (fix wave 2, correcting this section; gate-idx-rs MINOR). An earlier draft pinned `fnmatch.fnmatchcase` here, which does not implement the `**` this spec's own defaults are written in: `fnmatchcase("fleet.py", "**/*.py")` is `False` *and* `fnmatchcase("node_modules/x.py", "**/node_modules/**")` is `False`, so §8's default include would have skipped every top-level source file and its default exclude would have excluded nothing. The shipped matcher — already in use by the `fleet index` selection — reads `**/` as an optional directory prefix, `**` as spanning separators, and `*`/`?` as not crossing `/`; it is **case-sensitive on every platform**, which is what `fnmatchcase` was named for against `fnmatch.fnmatch` (the latter case-folds via `os.path.normcase` on win32 and would make `fleet q "beta.*"` match `Beta.run` on Windows but not on Linux). Form 3 name globs and the `--path` filter both use it, so `fleet q` and `fleet index` cannot come to speak two glob dialects.
...
- Config: absent config = defaults; `init` always writes the ignore entry, into `$GIT_COMMON_DIR/info/exclude` and never into the tracked `.gitignore` (§8) — pinned by a git-backed test that adds a real worktree, runs `init`, and asserts `git status --porcelain` is empty and `git worktree remove` succeeds; an unrecognised key (including the reserved `mode`) is a loud config error.
- Containment, written as **attacks**: a real file and a real directory are planted outside the index root, every traversal shape is aimed at them through each entry surface (the choke point, `update_index`, the `--files` CLI, `shard_path_for_source`), and the assertion is that **the file is still there** — not that an exit code was non-zero. The measured defect returned a non-zero code with a note that read like ordinary housekeeping, so an exit-code assertion passes against the deleting build. A junctioned mirror directory inside the shard tree is one of the shapes, because lexical containment passes it. **Three shapes beyond `..`, each pinned separately because each defeats a different guard:** a drive-qualified rel (defeats the `..` check — no `..` in it); a drive-*relative* rel (defeats `is_absolute()`, pinned alongside the two-line measurement showing `PureWindowsPath('C:x.py').is_absolute()` is False while `Path('D:/root') / 'C:x.py'` is `C:x.py`); and a reparse point in the SOURCE tree reached through the read and write choke points (defeats every string guard). Each attack asserts the victim's bytes are unchanged and no shard was materialised outside the root. Two counterweights carry equal weight, because a guard that refuses everything passes every attack above: an ordinary deep rel still indexes end-to-end, and an index root reached *through* a link still contains its own files.
- The canonicaliser's universality is a claim the tests hold to: every caller-supplied rel — through `verified_shard_rows`, `update_index` and `_index_refresh_one` reached directly — is asserted to reach the shard the *canonical* rel names, for `./a.py`, `a\b.py` and `a/./b.py`. The claim was documented before it was true.
- `_index_is_reparse_point`'s own invariants, not only its use sites: a junction answers True *while `os.path.islink` answers False* (asserted as a conjunction, and skipped rather than counted as evidence on a platform that made a real symlink instead), an unstattable entry answers True, and — the counterweight — an ordinary file and directory answer False.
- Exit codes: a partial failure indexes what it can, leaves the failed file's old shard **stale rather than torn**, and exits 6; a refusal exits 1; the two are asserted *different* in one test, and 6 is asserted distinct from every other code the CLI publishes. The mixed indexed-and-failed case is pinned with a per-file injection, not a blanket one — a blanket `write_shard_atomic → False` fails every file, so `return 0 if indexed else RC` passed the whole exit-code class.
- One read of the source per refresh: a write forced into the window between the hash and the parse (deterministic), and a concurrent-writer thread flipping one file between two versions while the choke point refreshes it (natural reproduction, no injection) — in both cases no persisted shard may carry one version's header over the other's rows, on the `q` path *and* the build path.
- Row order: the rows a caller gets back are identical whether the shard was cached or just refreshed, on a source with a `(line, end)` tie.
...
| 2b | Query-time refresh dirties a tracked file, breaking `git worktree remove` | HIGH | **Accepted; re-dispositioned 2026-07-23, then CORRECTED 2026-07-27.** The 2026-07-23 disposition (operator sub-decision (a), council round 2) removed tracked mode entirely — committing `.fleet-index/` is documented-unsupported (§8) — and concluded *"so the dirty-tracked-file hazard cannot arise."* **That conclusion is measured false.** It reasoned only about the *refresh* path and missed `init`: `_ensure_index_gitignore` wrote to the tracked `.gitignore`, and §8 mandates `fleet index init --path <worktree>` for every campaign worktree, so every worktree fleet creates was left dirty and `git worktree remove` refused it. The finding's original severity was right; the disposition's reasoning was too narrow. Repaired in fix wave 2 by writing the entry to `$GIT_COMMON_DIR/info/exclude` (§8), pinned by a git-backed test that creates a real worktree, runs `init`, and asserts `git worktree remove` still succeeds. The refuse-on-stale branch this finding originally produced stays deleted with the mode; per the dissent's binding process note, this row — not deleted text — is the record. |
...
- **2026-07-27 — fix wave 3 (gate ESCALATE on `idx/core` @ `3eb8e5c`).** Wave 2 claimed the traversal defect closed; the **delete** half was, and the read/write half was not. §9 gains two rows: the partial-failure exit code (a distinct 6, breaking the collision wave 2's own minor introduced by routing `failed > 0` onto the 1 every refusal already carried), and the containment contract stated as the two questions it actually is — spelling and filesystem — including *why* the spelling test is `drive or root` rather than `is_absolute()`, which is a measured distinction and not a style preference. §12 gains the three non-`..` attack shapes, the two counterweights that keep a refuse-everything guard from passing them, the canonicaliser-universality claim as an asserted property rather than a comment, `_index_is_reparse_point`'s own invariants, and the per-file (not blanket) exit-code injection. No §5/§6/§8/§11 change: the shard format, the parsers and the query contract are untouched by this wave.
- **2026-07-27 — fix wave 2 (dual-lens gate on the built M1 shard layer, `idx/core` @ `077175e`).** Both lenses returned ESCALATE; this entry records the spec edits, not the code. §5: header hash widened `<sha256-8>` → `<sha256-16>` against a measured 32-bit collision, row order pinned to `(line, end, name)` as *both* the byte order and the returned-row order, parse-time escaping stated for `name` as well as `sig`, and the line-terminator rule (`\r\n` | `\r` | `\n`) written down. §6: the `tree.body`-only reading of "module-level" stated rather than left ambiguous, and the build-side enumeration fence (nested repository boundary + reparse points) specified. §8: the ignore entry moves from the tracked `.gitignore` to `$GIT_COMMON_DIR/info/exclude`. §11.1/§11.2: the glob dialect re-specified onto the shipped `**`-aware matcher — the pinned `fnmatch.fnmatchcase` matched neither §8 default. §13 finding 2b re-dispositioned: its "the dirty-tracked-file hazard cannot arise" is measured false and is corrected in place.
  **Authority note.** This spec is `ready-for-gate`, **not ratified**. A measured design correction to a ready-for-gate spec is in-bounds build work; ratification remains the operator's, later, regardless of what lands here. Nothing in this wave adds or promotes a Status line.
