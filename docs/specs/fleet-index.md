...
```
   ...
   **The `# volatile` marker's price, disclosed.** A volatile block is a WARN in `tools/verify_receipts.py` and is **not executed at all** by `tests/test_receipts.py` (`skip_volatile=True`, which is what keeps that suite hermetic). Measured: corrupting the treatment arm of the block above to `PROBE-OK-9999` produced `VERDICT: pass -- 0 failure(s), 1 warning(s)` and left the suite green. So **this one block is not machine-enforced** — it is a citation whose integrity rests on the reviewer, exactly like the prose around it. That is the honest cost of evidence that lives outside the repo, and it is why the five *tree-side* blocks in this document are deliberately not volatile: they carry the enforcement.
