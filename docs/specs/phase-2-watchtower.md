...
<!-- F8 -->
**READER/ACTION constraint (F8 = review M9 — rewritten):** watchtower is a reader for RULE EVALUATION; every ACTION (send, respawn) executes only via the same lock-guarded fleet.py command paths as the CLI — a direct function call within a short-lived lock scope, never holding the lock across the poll loop — inheriting pre-claim, sticky-dead, `--force` guards, and universal drain semantics. On guard/kill FAILURE (pre-claim rejected, PID kill fails, lock unavailable): emit an event, back off, and count the attempt toward the respawn-loop cap; never retry-spin holding the lock. This supersedes the old "READER of logs/registry plus single-writer for events.jsonl" wording: the single writer of `fleet.json` stays `fleet.py` under `fleet.lock` (invariant 6); events.jsonl is a lock-guarded append by whichever `fleet.py` invocation runs.
