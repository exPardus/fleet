...
2. PID+ctime liveness without psutil: Windows (PowerShell `Get-Process`? wmic deprecated), Linux (`/proc/<pid>/stat` field 22), macOS (`ps -o lstart= -p`). Precision differs — what tolerance?
   <!-- F15 -->
   **Carry (F15/M4):** this OQ must cover not just per-OS probe precision but query-FAILURE semantics — **per-OS probe failure modes carried into OQ2 (alive-unknown tier, query-failure direction)**. Per SPEC §4 (as amended by the v2.1 state-machine amendment, F15/B1 — spec-amend-1-state), a query that returns "exists but unreadable" (e.g. higher-integrity process, `.StartTime` throws `Win32Exception`; `/proc/<pid>` EPERM; `ps` access-denied) resolves to the alive-unknown tier — never demoted to `dead`, never respawn/kill-eligible without `--force`; missing/corrupt stored ctime or any transient query failure is not-live but retries once before any `working`→`dead` transition. **UNBUILT — this three-way-probe / alive-unknown tier is the C2 probe END-STATE (owned by C2 hardening kernel item 9), not shipped §4 behavior today; SPEC §4/F20 tag it `[UNBUILT — owned by C2 hardening kernel item 9]`. This OQ carries the per-OS shape of that end-state, not a description of current code.** OQ2 names the per-OS discriminator for each of the three states and the accepted residual per OS.
