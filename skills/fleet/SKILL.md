...
- **You may only retire your own workers.** `kill`, `clean` and `respawn` refuse a worker spawned by a
  ...
  but the next `fleet autoclean` husk sweep — a supervisor beat, a startup ritual, or a manual run —
  `claude rm`s that session too
