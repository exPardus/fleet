...
- **Succession is a two-step maneuver and both steps are fleet's.** `fleet sup-release` → fresh `fleet sup-spawn`. **The old middle step is gone**: `sup-release` now tombstones the releasing body's own registry record, so `_releaser_body_is_tombstoned` answers true, the released-claim refusal does not arm, and a supervisor **can** complete its own stand-down. Nobody stops the retired body to make succession work — do stop it anyway to reclaim the session, but the next `sup-boot` no longer waits on you. Handoff dispatch (`sup-handoff-begin`) has a measured record, not a rumour: **10 successors dispatched under the old `dontask` default were stillborn** (2026-07-24 / -26 / -27) and **7 dispatched under `bypass` booted and completed their handoff** 34–64 s after dispatch. 17/17, from `state/events.jsonl` cross-checked against `HANDOFF-COMPLETE` in `supervisor/JOURNAL.md`. Root cause found and fixed 2026-07-27 (`SUCCESSOR_DEFAULT_MODE` → `bypass`, plus the sid and turn stamps the failure hid behind). **No live drill has run under the fixed default yet**, so it is proven by the seven that already worked and by pins, not by a drill — prefer release-then-spawn until someone drives one green.
