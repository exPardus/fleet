...
Exit code `0` on both `--help` probes is not evidence of a working `pin` command — it's an artifact of commander printing help and exiting 0 whenever `--help` is present, regardless of whether the preceding word was a real subcommand. Taken together with the docs-lane finding (secondary, corroboration only) that pin is documented as an **agents-view TUI action, `Ctrl+T`**, described as making the session "keep its process running while idle" — there is no `claude ... pin` flag, subcommand, or `--json`-adjacent option anywhere in the scriptable surface. **A fleet overlay cannot pin a session non-interactively; if pinning matters to the design, it requires either driving the interactive TUI (out of scope for a headless daemon-driven overlay) or is simply unavailable to the automation.**
...
**Evidence discipline.** All quoted material above is copied verbatim from Phase A's report (`.superpowers/sdd/task-7-report.md`) and this phase's own `reap_watch.log` read + live roster pull; methods are shown (watcher script content already on record in Phase A, live-check recipe named); the one substantive UNOBSERVED (live-idle-session process reap) is named rather than glossed into either verdict.
